interface AttendanceStatusBadgeProps {
  status: 'PRESENT' | 'LATE' | 'HALF_DAY' | 'ABSENT' | 'ON_LEAVE' | 'HOLIDAY' | 'WEEK_OFF' | 'PENDING';
  lateMinutes?: number;
}
...
export function AttendanceStatusBadge({ status, lateMinutes }: AttendanceStatusBadgeProps) {
  const statusConfig = {
    ...
    LATE: { color: 'bg-yellow-100 text-yellow-800', label: lateMinutes ? `Late (${lateMinutes}m)` : 'Late' },
