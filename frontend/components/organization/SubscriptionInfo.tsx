    ...
    <div className="space-y-6">
      ...
      {isPastDue && (
        <div className="rounded-lg bg-red-50 border border-red-200 p-4">
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              <p className="text-sm font-medium text-red-900">Payment Past Due</p>
      ...
      <div className="border border-gray-200 rounded-lg p-6 bg-gradient-to-br from-blue-50 to-white">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h4 className="text-2xl font-bold text-gray-900">{subscription.planType} Plan</h4>
            <div className={`flex items-center gap-2 mt-2 ${getStatusColor()}`}>
              {getStatusIcon()}
              <span className="text-sm font-medium">{subscription.paymentStatus}</span>
            </div>
          </div>
          {subscription.monthlyPrice !== undefined && subscription.monthlyPrice !== null && (
            <div className="text-right">
              <div className="text-3xl font-bold text-gray-900">${subscription.monthlyPrice}</div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="border border-gray-200 rounded-lg p-4">
          ...
          <p className="text-gray-900 font-medium">{subscription.paymentStatus}</p>
        </div>
      </div>
      ...
      {subscription.endDate && (
        <div className="text-xs text-gray-500 text-center pt-4 border-t">
          Your subscription will automatically renew on {formatDate(subscription.endDate)}. You can
