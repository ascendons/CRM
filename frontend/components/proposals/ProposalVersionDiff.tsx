...
export default function ProposalVersionDiff({
  version1,
  version2,
  onBack,
}: ProposalVersionDiffProps) {
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                ...
                <dd
                  className={`text-sm font-medium ${v1.title !== v2.title ? "text-primary" : "text-slate-900"}`}
                >
                  {v2.title}
                </dd>
                <DiffBadge
                  status={getDiffStatus(v1.title, v2.title)}
                  value1={v1.title}
                  value2={v2.title}
                />
              </div>
              <div>
                ...
                <dd
                  className={`text-sm font-medium ${v1.status !== v2.status ? "text-primary" : "text-slate-900"}`}
                >
                  {v2.status}
                </dd>
                <DiffBadge
                  status={getDiffStatus(v1.status, v2.status)}
                  value1={v1.status}
                  value2={v2.status}
                />
              </div>
              <div className="md:col-span-2">
                ...
                <dd
                  className={`text-sm font-medium ${v1.description !== v2.description ? "text-primary" : "text-slate-900"}`}
                >
                  {v2.description || "None"}
                </dd>
                <DiffBadge
                  status={getDiffStatus(v1.description, v2.description)}
                  value1={v1.description}
                  value2={v2.description}
                />
              </div>
            </div>
          </div>
              ...
              <table className="w-full text-left">
                ...
                <tbody className="divide-y divide-slate-100">
                  {v2.lineItems.map((item2) => {
                    ...
                    return (
                      <tr
                        key={item2.lineItemId}
                        className={isNew ? "bg-emerald-50/30" : isChanged ? "bg-amber-50/30" : ""}
                      >
                        <td className="px-6 py-4">
                          ...
                          {isNew && (
                            <span className="mt-1 inline-block px-1.5 py-0.5 rounded bg-emerald-100 text-emerald-800 text-[10px] font-bold uppercase">
                              New
                            </span>
                          )}
                        </td>
                        <td className="px-6 py-4 text-right">
                          <div
                            className={`text-sm font-medium ${item1?.quantity !== item2.quantity ? "text-primary font-bold" : "text-slate-900"}`}
                          >
                            {item2.quantity} {item2.unit}
                          </div>
                          <DiffBadge
                            status={getDiffStatus(item1?.quantity, item2.quantity)}
                            value1={item1?.quantity}
                            value2={item2.quantity}
                          />
                        </td>
                        <td className="px-6 py-4 text-right">
                          <div
                            className={`text-sm font-medium ${item1?.unitPrice !== item2.unitPrice ? "text-primary font-bold" : "text-slate-900"}`}
                          >
                            {formatCurrency(item2.unitPrice)}
                          </div>
                          <DiffBadge
                            status={getDiffStatus(item1?.unitPrice, item2.unitPrice)}
                            value1={item1?.unitPrice}
                            value2={item2.unitPrice}
                          />
                        </td>
                        <td className="px-6 py-4 text-right">
                          <div
                            className={`text-sm font-bold ${item1?.lineTotal !== item2.lineTotal ? "text-primary" : "text-slate-900"}`}
                          >
                            {formatCurrency(item2.lineTotal)}
                          </div>
                          <DiffBadge
                            status={getDiffStatus(item1?.lineTotal, item2.lineTotal)}
                            value1={item1?.lineTotal}
                            value2={item2.lineTotal}
                          />
                        </td>
                      </tr>
                    );
                  ...
                  {v1.lineItems
                    .filter((i1) => !v2.lineItems.find((i2) => i2.productId === i1.productId))
                    .map((item1) => (
                      <tr key={item1.lineItemId} className="bg-rose-50/30 grayscale-[0.5]">
                        <td className="px-6 py-4 opacity-70">
                          <div className="text-sm font-bold text-slate-900 line-through">
                            {item1.productName}
                          </div>
                          <div className="text-xs text-slate-500">SKU: {item1.sku}</div>
                          <span className="mt-1 inline-block px-1.5 py-0.5 rounded bg-rose-100 text-rose-800 text-[10px] font-bold uppercase">
                            Removed
                          </span>
                        </td>
        ...
        <div className="space-y-6">
          <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 overflow-hidden relative">
            ...
            <h3 className="text-sm font-bold text-slate-400 uppercase tracking-wider mb-6 relative z-10">
              Financial Impact
            </h3>
            ...
            <dl className="space-y-6 relative z-10">
              <div>
                ...
                <DiffBadge
                  status={getDiffStatus(v1.subtotal, v2.subtotal)}
                  value1={v1.subtotal}
                  value2={v2.subtotal}
                />
              </div>
                ...
                <dt className="text-xs font-bold text-slate-500 mb-1 flex items-center justify-between">
                  ...
                  {v2.discount && (
                    <span className="text-[10px] px-1.5 py-0.5 bg-slate-100 rounded text-slate-600">
                      {v2.discount.overallDiscountType === DiscountType.PERCENTAGE
                        ? `${v2.discount.overallDiscountValue}%`
                        : "Fixed"}
                    </span>
                  )}
                </dt>
                <dd
                  className={`text-base font-bold ${v2.discountAmount > 0 ? "text-rose-600" : "text-slate-900"}`}
                >
                  -{formatCurrency(v2.discountAmount)}
                </dd>
                <DiffBadge
                  status={getDiffStatus(v1.discountAmount, v2.discountAmount)}
                  value1={v1.discountAmount}
                  value2={v2.discountAmount}
                />
              </div>
              ...
              <div>
                ...
                <DiffBadge
                  status={getDiffStatus(v1.taxAmount, v2.taxAmount)}
                  value1={v1.taxAmount}
                  value2={v2.taxAmount}
                />
              </div>
              ...
              <div className="pt-6 border-t border-slate-100 mt-2">
                <dt className="text-xs font-bold text-primary uppercase tracking-tighter mb-1">
                  Final Total
                </dt>
                ...
                <DiffBadge
                  status={getDiffStatus(v1.totalAmount, v2.totalAmount)}
                  value1={v1.totalAmount}
                  value2={v2.totalAmount}
                />
              </div>
            </dl>
          </div>
