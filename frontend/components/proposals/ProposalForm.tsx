...
export default function ProposalForm({
  ...
  const [sourceId, setSourceId] = useState(initialData?.sourceId || initialSourceId || "");
  ...
  useEffect(() => {
    if (
      mode === "create" &&
      !hasInitiallyPopulated &&
      leads.length > 0 &&
      source === ProposalSource.LEAD &&
      sourceId
    ) {
  ...
  const [lineItems, setLineItems] = useState<FormLineItem[]>(
    initialData?.lineItems.map((item) => ({
      ...
      listPrice: (item as any).listPrice,
  ...
  const [hasOverallDiscount, setHasOverallDiscount] = useState(!!initialData?.discount);
  ...
  const [discountReason, setDiscountReason] = useState(initialData?.discount?.discountReason || "");

  // GST state
  const [gstType, setGstType] = useState<GstType>(initialData?.gstType || GstType.NONE);
  ...
  const [status, setStatus] = useState<ProposalStatus>(initialData?.status || ProposalStatus.DRAFT);
    ...
    try {
      ...
      if (orgData?.settings) {
        if (!paymentTerms || paymentTerms.trim() === "")
          setPaymentTerms(orgData.settings.defaultPaymentTerms || "");
        if (!deliveryTerms || deliveryTerms.trim() === "")
          setDeliveryTerms(orgData.settings.defaultDeliveryTerms || "");
  ...
  const addMilestone = () => {
    setPaymentMilestones([...paymentMilestones, { name: "", percentage: 0 }]);
  };
  ...
  const updateMilestone = (
    index: number,
    field: keyof FormPaymentMilestone,
    value: string | number
  ) => {
  ...
  const validateForm = () => {
    ...
    if (new Date(validUntil) <= new Date()) errors.push("Valid until date must be in the future");
    ...
    if (
      lineItems.some(
        (item) => !item.productId && (item.unitPrice === undefined || item.unitPrice === null)
      )
    )
    ...
    if (paymentMilestones.length > 0) {
      const totalPercentage = paymentMilestones.reduce(
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Show confirmation modal on create to prevent duplicate submissions
    if (mode === "create") {
  ...
  const doSubmit = async () => {
    ...
    try {
      ...
      if (mode === "create") {
        ...
        const shouldRequest =
          document.getElementsByName("requestApproval")[0]?.getAttribute("value") === "true";
        ...
        const updated = await proposalsService.updateProposal(initialData.id, request);
      ...
      showToast.error(err instanceof Error ? err.message : `Failed to ${mode} proposal`);
    } finally {
      setLoading(false);
    }
  };
  ...
  const isReadOnly =
    mode === "edit" &&
    (initialData?.status === ProposalStatus.ACCEPTED ||
      initialData?.status === ProposalStatus.REJECTED);
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <div>
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    onClick={() => {
                      setIsProforma(false);
                      setIsTechnicalQuotation(false);
                    }}
                    className={`flex-1 py-2 px-3 text-sm border rounded-lg font-medium transition-colors ${!isProforma && !isTechnicalQuotation ? "bg-blue-600 text-white border-blue-600" : "bg-white text-gray-700 border-gray-300 hover:bg-gray-50"}`}
                  >
                    Quotation
                  </button>
                  <button
                  ...
                  <button
                    ...
                    onClick={() => {
                      setIsProforma(false);
                      setIsTechnicalQuotation(true);
                    }}
                    className={`flex-1 py-2 px-3 text-sm border rounded-lg font-medium transition-colors ${isTechnicalQuotation ? "bg-purple-600 text-white border-purple-600" : "bg-white text-gray-700 border-gray-300 hover:bg-gray-50"}`}
                  >
                    Technical Quotation
                  </button>
                </div>
              ...
              <div className="space-y-3">
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setBillingAddress({ ...billingAddress, gstNumber: e.target.value })
                    }
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 text-sm disabled:bg-gray-100"
                    disabled={isReadOnly}
                ...
                {!isShippingSameAsBilling && (
                  <div className="space-y-3 animate-in fade-in slide-in-from-top-2 duration-200">
                    ...
                    <CountryStateSelector
                      ...
                      onStateChange={(val) =>
                        setShippingAddress((prev) => ({ ...prev, state: val }))
                      }
                      disabled={isReadOnly}
                      labelClassName="block text-xs font-medium text-gray-500 mb-1"
                    />
          ...
          {lineItems.map((item, index) => (
            <div key={index} className="border border-gray-200 rounded-lg p-3 bg-gray-50 space-y-2">
              ...
              <div className="flex flex-col lg:flex-row items-start gap-2">
                {/* Mobile label */}
                <span className="lg:hidden text-xs font-semibold text-gray-500">
                  Item #{index + 1}
                </span>
                    ...
                    onSelect={(product) => {
                      setLineItems((prev) => {
                        ...
                        const getAttr = (keys: string[]) =>
                          product.attributes.find((a) => keys.includes(a.key.toLowerCase()));
                        ...
                        const listPriceAttr = getAttr(["listprice", "list_price", "mrp"]);
                        if (listPriceAttr) {
                          if (listPriceAttr.numericValue != null) {
                            newUpdates.listPrice = listPriceAttr.numericValue;
                          } else if (listPriceAttr.value) {
                            newUpdates.listPrice = parseFloat(listPriceAttr.value);
                          }
                        }
                        // Fall back to unitPrice if no explicit list price
                        if (!newUpdates.listPrice && newUpdates.unitPrice) {
                          newUpdates.listPrice = newUpdates.unitPrice;
                        }
                ...
                <div className="w-28 flex-shrink-0">
                  <select
                    ...
                    onChange={(e) =>
                      updateLineItem(index, "discountType", e.target.value || undefined)
                    }
                    className={inputCls}
                    disabled={isReadOnly}
                  >
              ...
              {paymentMilestones.length > 0 ? (
                <>
                  <div className="flex items-center justify-between text-xs">
                    ...
                    <span
                      className={`font-semibold ${milestoneTotal === 100 ? "text-green-600" : "text-red-600"}`}
                    >
                ...
                <p className="text-sm text-gray-400 italic">
                  No milestones. Proposal is 100% upfront.
                </p>
              )}
      ...
      {/* Confirm create modal to prevent duplicate submissions */}
      <ConfirmModal
        isOpen={showConfirmModal}
        title={
          isTechnicalQuotation
            ? "Create Technical Quotation"
            : isProforma
              ? "Create Proposal"
              : "Create Quotation"
        }
        message={`You are about to create a ${isTechnicalQuotation ? "Technical Quotation" : isProforma ? "Proposal" : "Quotation"} with ${lineItems.length} line item(s). Do you want to proceed?`}
        confirmLabel="Create"
        cancelLabel="Cancel"
