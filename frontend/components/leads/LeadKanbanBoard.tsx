...
function KanbanColumn({
  ...
  return (
    <div
      ref={setNodeRef}
      className="flex-shrink-0 w-80 flex flex-col bg-slate-50/50 rounded-2xl border border-slate-200/60 max-h-full"
    >
      {/* Column Header */}
      <div
        className={`p-4 rounded-t-2xl border-b ${color.includes("bg-") ? color.replace("text-", "border-").replace("50", "200") : "border-slate-200"} bg-white sticky top-0 z-10`}
      >
...
export function LeadKanbanBoard({
  ...
  const visibleColumns = statusColumns.filter(
    (column) => filter.length === 0 || filter.includes(column.id)
  );
  ...
  const handleHookModalClose = () => {
    ...
    if (pendingStatusChange) {
      ...
      if (originalLead) {
        setLeads((prev) =>
          prev.map((l) =>
            l.id === pendingStatusChange.leadId ? { ...l, leadStatus: originalLead.leadStatus } : l
          )
        );
      }
    }
