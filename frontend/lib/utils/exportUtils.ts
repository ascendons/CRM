  ...
  const rows = [
    ...
    [
      ...
      report.totalWorkHours
        ? `${Math.floor(report.totalWorkHours / 60)}h ${report.totalWorkHours % 60}m`
        : "-",
    ],
    [
      "Average Work Hours",
      report.averageWorkHours
        ? `${Math.floor(report.averageWorkHours / 60)}h ${report.averageWorkHours % 60}m`
        : "-",
    ],
    [
      "Total Overtime",
      report.totalOvertime
        ? `${Math.floor(report.totalOvertime / 60)}h ${report.totalOvertime % 60}m`
        : "-",
    ],
