...
export const fieldService = {
  ...
  createAssetCategory: (data: Partial<AssetCategory>) =>
    api.post<AssetCategory>("/asset-categories", data),
  updateAssetCategory: (id: string, data: Partial<AssetCategory>) =>
  ...
  getAllContracts: (params?: { accountId?: string; expiringSoonDays?: number }) => {
    ...
    if (params?.expiringSoonDays)
      query.append("expiringSoonDays", params.expiringSoonDays.toString());
  ...
  renewContract: (id: string, data: CreateContractRequest) =>
    api.post<Contract>(`/contracts/${id}/renew`, data),
  updateContract: (id: string, data: CreateContractRequest) =>
    api.put<Contract>(`/contracts/${id}`, data),
  getContractVisits: (id: string) => api.get<ContractVisit[]>(`/contracts/${id}/visits`),
  updateContractVisit: (visitId: string, data: Partial<ContractVisit>) =>
    api.put<ContractVisit>(`/contracts/visits/${visitId}`, data),
  ...
  acknowledgeServiceRequest: (id: string) =>
    api.post<ServiceRequest>(`/service-requests/${id}/acknowledge`, {}),
  closeServiceRequest: (id: string) =>
    api.post<ServiceRequest>(`/service-requests/${id}/close`, {}),

  // Work Orders
  getAllWorkOrders: (params?: {
  ...
  assignWorkOrder: (id: string, engineerIds: string[]) =>
    api.post<WorkOrder>(`/work-orders/${id}/assign`, { engineerIds }),
  updateWorkOrderStatus: (id: string, status: WorkOrderStatus) =>
    api.post<WorkOrder>(`/work-orders/${id}/status`, { status }),
  ...
  createChecklistTemplate: (data: Partial<Checklist>) =>
    api.post<Checklist>("/checklists/templates", data),
  updateChecklistTemplate: (id: string, data: Partial<Checklist>) =>
    api.put<Checklist>(`/checklists/templates/${id}`, data),
  deleteChecklistTemplate: (id: string) => api.delete<void>(`/checklists/templates/${id}`),

  getChecklistResponse: (woId: string) =>
    api.get<ChecklistResponse>(`/checklists/responses/work-order/${woId}`),
  ...
  getInventoryTopConsumed: (limit = 20) =>
    api.get<any>(`/analytics/service/inventory/top-consumed?limit=${limit}`),
  ...
  updateEscalationRule: (id: string, data: any) =>
    api.put<any>(`/admin/settings/escalation/rules/${id}`, data),
  ...
  acknowledgeEscalation: (logId: string) =>
    api.post<void>(`/admin/settings/escalation/logs/${logId}/acknowledge`, {}),
  resolveEscalation: (logId: string) =>
    api.post<void>(`/admin/settings/escalation/logs/${logId}/resolve`, {}),
};
