  ...
  const getStatusBadge = (status: AssetStatus) => {
    switch (status) {
      ...
      case AssetStatus.UNDER_REPAIR:
        return <Badge className="bg-amber-50 text-amber-700 border-amber-100 font-bold uppercase text-[10px]">Under Repair</Badge>;
      case AssetStatus.DECOMMISSIONED:
        return <Badge className="bg-slate-100 text-slate-700 border-slate-200 font-bold uppercase text-[10px]">Decommissioned</Badge>;
      default:
    ...
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4">
        <div className="text-center max-w-md bg-white p-8 rounded-3xl shadow-sm border border-slate-200">
          ...
          <p className="text-slate-500 mb-8 font-medium">{error || "The requested equipment record could not be found."}</p>
            ...
            <div className="flex items-center gap-5">
              ...
              <div className="space-y-1">
                <div className="flex items-center gap-3">
                  <h1 className="text-2xl font-black text-slate-900 tracking-tight font-mono">{asset.assetCode}</h1>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <Tabs defaultValue="overview" className="space-y-8">
          <TabsList className="bg-white p-1 rounded-2xl border border-slate-200 shadow-sm inline-flex h-14 w-full md:w-auto">
            <TabsTrigger value="overview" className="rounded-xl px-8 font-bold data-[state=active]:bg-primary/5 data-[state=active]:text-primary transition-all">
              <FileText className="h-4 w-4 mr-2" />
              Overview
            </TabsTrigger>
            <TabsTrigger value="history" className="rounded-xl px-8 font-bold data-[state=active]:bg-primary/5 data-[state=active]:text-primary transition-all">
              <History className="h-4 w-4 mr-2" />
              Service History
              {workOrders.length > 0 && <span className="ml-2 bg-primary/10 px-2 py-0.5 rounded-full text-[10px]">{workOrders.length}</span>}
            </TabsTrigger>
            <TabsTrigger value="contract" className="rounded-xl px-8 font-bold data-[state=active]:bg-primary/5 data-[state=active]:text-primary transition-all">
              <ShieldCheck className="h-4 w-4 mr-2" />
              Contract Info
            </TabsTrigger>
          </TabsList>

          <TabsContent value="overview" className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 focus:outline-none outline-none">
              ...
              <Card className="lg:col-span-2 rounded-3xl border-slate-200 shadow-sm overflow-hidden hover:shadow-md transition-shadow">
                ...
                <CardContent className="p-4 md:p-8">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-2 md:gap-4">
                    <DetailItem label="Asset Category" value={asset.categoryName} icon={Settings2} colorClass="text-primary" />
                    <DetailItem label="Registration Date" value={new Date(asset.createdAt).toLocaleDateString()} icon={Calendar} />
                    <DetailItem label="Installation Date" value={asset.installDate ? new Date(asset.installDate).toLocaleDateString() : "Pending Installation"} icon={Calendar} colorClass="text-emerald-500" />
                    <DetailItem label="Warranty Status" value={asset.warrantyExpiry ? `Expires ${new Date(asset.warrantyExpiry).toLocaleDateString()}` : "No Active Warranty"} icon={ShieldCheck} colorClass={asset.warrantyExpiry && new Date(asset.warrantyExpiry) < new Date() ? "text-rose-500" : "text-blue-500"} />
                  </div>
              ...
              <div className="space-y-8">
                <Card className="rounded-3xl border-slate-200 shadow-sm overflow-hidden hover:shadow-md transition-shadow">
                  ...
                  <CardContent className="p-6">
                    <div className="flex items-center gap-4 mb-6">
                      ...
                      <div>
                        <p className="text-md font-bold text-slate-900 tracking-tight">{asset.accountName || "Global Customer"}</p>
                        <Button 
                          variant="link" 
                ...
                <Card className="rounded-3xl border-slate-200 shadow-sm overflow-hidden hover:shadow-md transition-shadow bg-slate-900 border-none text-white">
                  <CardContent className="p-8">
                     <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-4">Assigned Engineer</p>
                     <div className="flex items-center gap-4">
                        <div className="h-10 w-10 rounded-full bg-primary/20 flex items-center justify-center text-primary font-bold">
                           {asset.assignedEngineerName?.[0] || "E"}
                        </div>
                        <div>
                           <p className="text-sm font-bold text-white tracking-tight">{asset.assignedEngineerName || "Unassigned"}</p>
                ...
                <CardContent className="p-0">
                   {workOrders.length === 0 ? (
                      <div className="p-20 text-center">
                         ...
                         <p className="text-sm text-slate-500 font-medium mb-8">This unit hasn't had any service calls or PM visits registered yet.</p>
                         <Button variant="outline" className="rounded-xl font-bold">Create First Service Call</Button>
                      </div>
                   ) : (
                         ...
                         <table className="w-full text-left">
                            <thead>
                               <tr className="border-b border-slate-100">
                                  <th className="px-6 py-4 text-[10px] font-bold text-slate-400 uppercase tracking-widest">WO Number</th>
                            ...
                            <tbody className="divide-y divide-slate-50">
                               {workOrders.map((wo) => (
                                  <tr key={wo.id} className="hover:bg-slate-50/50 transition-colors group cursor-pointer" onClick={() => router.push(`/work-orders/${wo.id}`)}>
                                     <td className="px-6 py-4 text-sm font-bold text-slate-900 font-mono tracking-tight group-hover:text-primary">{wo.woNumber}</td>
                                     <td className="px-6 py-4">
                                        <Badge variant="secondary" className="bg-slate-100 text-slate-600 rounded-lg text-[10px] font-bold uppercase">{wo.type.replace('_', ' ')}</Badge>
                                     </td>
                                     <td className="px-6 py-4 text-sm font-medium text-slate-600">{new Date(wo.createdAt).toLocaleDateString()}</td>
                                     <td className="px-6 py-4">
                                        <Badge className={`rounded-lg text-[10px] font-bold uppercase ${wo.status === 'COMPLETED' ? 'bg-emerald-50 text-emerald-700' : 'bg-amber-50 text-amber-700'}`}>
                                          {wo.status}
                                        </Badge>
                                     </td>
                                     <td className="px-6 py-4 text-right">
                                        <Button variant="ghost" size="sm" className="rounded-lg h-8 w-8 p-0 opacity-0 group-hover:opacity-100">
             ...
             <div className="max-w-3xl">
                <Card className="rounded-3xl border-slate-200 shadow-sm overflow-hidden">
                   <CardContent className="p-12 text-center bg-slate-50/50">
                        ...
                        <h2 className="text-2xl font-black text-slate-900 tracking-tight mb-4">Contract Protection</h2>
                        <p className="text-slate-500 font-medium leading-relaxed mb-8">
                           This asset is currently in its initial setup phase. Once linked to an active AMC or Service Contract, its protection details, SLA metrics, and billing cycles will appear here.
                        </p>
                        <Button
