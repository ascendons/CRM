  ...
  const getStatusBadge = (status: ContractVisitStatus) => {
    switch (status) {
      case ContractVisitStatus.SCHEDULED:
        return <Badge className="bg-blue-50 text-blue-700 border-blue-100 uppercase text-[10px] font-black">Scheduled</Badge>;
      case ContractVisitStatus.COMPLETED:
        return <Badge className="bg-emerald-50 text-emerald-700 border-emerald-100 uppercase text-[10px] font-black">Completed</Badge>;
      case ContractVisitStatus.MISSED:
        return <Badge className="bg-rose-50 text-rose-700 border-rose-100 uppercase text-[10px] font-black">Missed</Badge>;
      case ContractVisitStatus.RESCHEDULED:
        return <Badge className="bg-violet-50 text-violet-700 border-violet-100 uppercase text-[10px] font-black">Rescheduled</Badge>;
      case ContractVisitStatus.CANCELLED:
        return <Badge className="bg-slate-50 text-slate-700 border-slate-100 uppercase text-[10px] font-black">Cancelled</Badge>;
      default:
          ...
          <div className="flex items-center justify-between py-5">
            <div className="flex items-center gap-5">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500 font-bold uppercase tracking-widest">{contract?.contractNumber} • Prevention Visits</p>
              </div>
            </div>
            <div className="flex items-center gap-2 bg-white px-4 py-2 rounded-2xl border border-slate-200 shadow-sm">
                <div className="h-2 w-2 rounded-full bg-emerald-500 animate-pulse"></div>
                <span className="text-xs font-black text-slate-700 uppercase tracking-wider">{visits.length} Visits Allocated</span>
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-10">
          ...
          <div className="lg:col-span-1 space-y-6">
            <Card className="rounded-[2.5rem] border-none shadow-xl bg-slate-900 text-white overflow-hidden">
               <div className="p-8 space-y-6">
                  ...
                  <div>
                     <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Contract Validity</p>
                     <p className="text-sm font-bold">{new Date(contract?.startDate || "").toLocaleDateString()} — {new Date(contract?.endDate || "").toLocaleDateString()}</p>
                  </div>
                  <div className="p-4 bg-white/5 rounded-2xl border border-white/10">
                     <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Requirement</p>
                     <p className="text-xs font-medium leading-relaxed opacity-80">
                        Performing periodic maintenance reduces equipment downtime by up to 35% and maintains warranty eligibility.
                     </p>
                  </div>
               </div>
            </Card>
          ...
          <div className="lg:col-span-3 space-y-6">
            <div className="flex items-center justify-between gap-4 mb-2">
               ...
               <Button variant="outline" className="h-14 w-14 rounded-2xl border-none shadow-xl bg-white hover:bg-slate-50">
            ...
            <div className="grid grid-cols-1 gap-4">
              {filteredVisits.length === 0 ? (
                <div className="bg-white rounded-[2.5rem] p-20 text-center shadow-xl border-2 border-dashed border-slate-200">
                   ...
                   <h3 className="text-xl font-black text-slate-900 tracking-tight mb-2">Schedule Not Found</h3>
                   <p className="text-slate-500 font-medium">No maintenance visits have been initialized for this contract yet.</p>
                </div>
              ) : (
                filteredVisits.sort((a,b) => a.visitNumber - b.visitNumber).map((visit) => (
                  <Card key={visit.id} className="rounded-[2rem] border-none shadow-lg hover:shadow-2xl transition-all duration-300 bg-white group overflow-hidden">
                    <CardContent className="p-6">
                      <div className="flex flex-col md:flex-row items-center gap-6">
                         <div className="flex-shrink-0 w-24 h-24 bg-slate-50 rounded-3xl flex flex-col items-center justify-center border-2 border-slate-100 group-hover:border-primary/20 transition-colors">
                            <span className="text-[10px] font-black text-slate-400 uppercase">Visit</span>
                            <span className="text-3xl font-black text-slate-900 tracking-tighter">#{visit.visitNumber}</span>
                         </div>
                         ...
                         <div className="flex-1 space-y-2 text-center md:text-left">
                            <div className="flex flex-wrap items-center justify-center md:justify-start gap-3 mb-2">
                               ...
                               {visit.actualDate && (
                                  <Badge className="bg-slate-900 text-white uppercase text-[10px] font-black">Performed on {new Date(visit.actualDate).toLocaleDateString()}</Badge>
                               )}
                            </div>
                            <div className="flex items-center justify-center md:justify-start gap-6 text-slate-600">
                               <div className="flex items-center gap-2">
                                  <Calendar className="h-4 w-4 text-primary" />
                                  <span className="text-sm font-bold">{new Date(visit.scheduledDate).toLocaleDateString("en-US", { month: 'long', day: 'numeric', year: 'numeric' })}</span>
                               </div>
                               <div className="flex items-center gap-2">
                                  ...
                                  <span className="text-sm font-bold">
                                     {(() => {
                                        ...
                                        return eng ? (eng.profile?.fullName || eng.username) : "Unassigned Engineer";
                                     })()}
                                  </span>
                               </div>
                            </div>
                            {visit.notes && (
                               <p className="text-xs font-medium text-slate-400 italic">"{visit.notes}"</p>
                            )}
                         </div>

                         <div className="flex items-center gap-2">
                            {visit.status === ContractVisitStatus.SCHEDULED || visit.status === ContractVisitStatus.RESCHEDULED ? (
                               <Button 
                            ...
                            {visit.workOrderId && (
                               <Button variant="ghost" onClick={() => router.push(`/work-orders/${visit.workOrderId}`)} className="rounded-xl h-12 w-12 border border-slate-100 p-0 text-slate-400 hover:text-primary">
          ...
          <div className="bg-white rounded-[3rem] w-full max-w-lg overflow-hidden shadow-2xl animate-in zoom-in-95 duration-300 border-2 border-white">
            <div className="px-10 pt-10 pb-6 flex items-center justify-between">
               <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h2 className="text-2xl font-black text-slate-900 tracking-tight">Modify Schedule</h2>
                    <p className="text-xs font-bold text-slate-400 uppercase tracking-widest">Visit #{reschedulingVisit.visitNumber}</p>
                  </div>
               </div>
               <button onClick={() => setReschedulingVisit(null)} className="h-12 w-12 rounded-full hover:bg-slate-100 flex items-center justify-center transition-colors">
            ...
            <form onSubmit={submitReschedule} className="p-10 pt-4 space-y-8">
               <div className="space-y-6">
                  ...
                  <div className="space-y-3">
                    <Label className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Assign System Engineer</Label>
                    <select
                      value={rescheduleData.engineerId}
                      onChange={(e) => setRescheduleData({...rescheduleData, engineerId: e.target.value})}
                  ...
                  <div className="space-y-3">
                    <Label className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Change Notes</Label>
                    <textarea 
                      rows={3}
               ...
               >
                 {isSubmitting ? <Loader2 className="h-5 w-5 animate-spin mr-3" /> : <RefreshCw className="h-5 w-5 mr-3" />}
                 Apply Rescheduling
               </Button>
            </form>
          </div>
