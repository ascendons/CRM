...
export default function EditContractPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [formData, setFormData] = useState<CreateContractRequest & { id?: string }>({
  ...
  const loadInitialData = async () => {
    try {
      ...
      const [contractData, accountsData] = await Promise.all([
      ...
      // Load assets for the contract's account
      if (contractData.accountId) {
        const assets = await fieldService.getAllAssets({ accountId: contractData.accountId });
      ...
      setFormData({
        ...
        assetIds: contractData.assetIds || [],
        ...
        billingCycle: contractData.billingCycle,
        visitFrequencyPerYear: contractData.visitFrequencyPerYear,
        contractValue: contractData.contractValue,
        slaConfig: {
          responseHrs: contractData.slaConfig?.responseHrs || 4,
          resolutionHrs: contractData.slaConfig?.resolutionHrs || 24
        },
        penaltyConfig: {
          perHourBreachPenalty: contractData.penaltyConfig?.perHourBreachPenalty || 0,
          maxPenaltyCap: contractData.penaltyConfig?.maxPenaltyCap || 0
        },
  ...
  // Only load new assets if the account actually changes from the initial load
  const handleAccountChange = async (e: React.ChangeEvent<HTMLSelectElement>) => {
    const accountId = e.target.value;
    setFormData(prev => ({ ...prev, accountId, assetIds: [] }));
    await loadAssetsForAccount(accountId);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { id: _, ...requestData } = formData;
      await fieldService.updateContract(id, requestData as CreateContractRequest);
    ...
    return (
      <div className="flex items-center justify-center bg-slate-50 min-h-[calc(100vh-4rem)]">
        <div className="text-center space-y-4">
          ...
          <p className="text-slate-500 font-medium tracking-tight">Retrieving contract details...</p>
        </div>
      </div>
    );
  }
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between py-5">
            <div className="flex items-center gap-5">
              ...
              <div>
                <h1 className="text-2xl font-black text-slate-900 tracking-tight">Edit Service Contract</h1>
                <p className="text-xs text-slate-500 font-bold uppercase tracking-widest px-1">Ref: {formData.id?.split('-')[0].toUpperCase()}</p>
              </div>
            </div>
            <div className="flex items-center gap-3">
              ...
              <Button
                ...
                disabled={loading}
                className="rounded-2xl px-8 font-black shadow-xl shadow-primary/25 bg-primary hover:bg-primary/90 transition-all active:scale-95"
              >
              ...
              <Card className="rounded-[32px] border-slate-200 shadow-xl overflow-hidden overflow-visible border-none bg-white p-2">
                <CardHeader className="px-8 pt-8 pb-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                       <CardTitle className="text-xl font-black tracking-tight text-slate-900">Account Configuration</CardTitle>
                       <CardDescription className="font-medium text-slate-400">Customer linkage and linked equipment registry.</CardDescription>
                    </div>
                  </div>
                </CardHeader>
                <CardContent className="px-8 pb-8 pt-4">
                  <div className="space-y-6">
                    <div className="space-y-3">
                      <Label htmlFor="accountId" className="text-slate-500 font-black text-[10px] uppercase tracking-[0.15em]">Primary Customer *</Label>
                      <select
                        id="accountId"
                        name="accountId"
                        required
                    ...
                    <div className="pt-6 border-t border-slate-50">
                      <Label className="text-slate-500 font-black text-[10px] uppercase tracking-[0.15em] block mb-4">Link Assets to Contract ({formData.assetIds.length} Selected)</Label>
                      {availableAssets.length === 0 ? (
                         <div className="bg-amber-50 rounded-2xl p-6 text-center border-2 border-amber-100">
                            ...
                            <p className="text-sm font-bold text-amber-900 tracking-tight">No assets found for this customer.</p>
                            <p className="text-xs text-amber-600 mt-1 font-medium">Please add equipment to the customer's registry first.</p>
                         </div>
                      ) : (
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 max-h-[300px] overflow-y-auto pr-2 custom-scrollbar">
                          {availableAssets.map((asset) => (
                            ...
                            >
                              <div className={`p-2 rounded-xl ${formData.assetIds.includes(asset.id) ? "bg-primary text-white" : "bg-slate-50 text-slate-400"}`}>
                                 <Laptop className="h-4 w-4" />
                              </div>
                              <div>
                                 <p className="text-sm font-black text-slate-900 tracking-tight">{asset.brand} {asset.model}</p>
                                 <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">{asset.serialNo}</p>
                              </div>
                            </div>
                          ))}
                        </div>
              ...
              <Card className="rounded-[32px] border-none shadow-xl bg-white p-2">
                <CardHeader className="px-8 pt-8 pb-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                       <CardTitle className="text-xl font-black tracking-tight text-slate-900">Service Terms & SLA</CardTitle>
                       <CardDescription className="font-medium text-slate-400">Modify performance commitments and billing.</CardDescription>
                    </div>
                  </div>
                </CardHeader>
                <CardContent className="px-8 pb-8 pt-4 space-y-8">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div className="space-y-3">
                      <Label className="text-slate-500 font-black text-[10px] uppercase tracking-[0.15em]">Contract Type</Label>
                    ...
                    <div className="space-y-3">
                      <Label htmlFor="billingCycle" className="text-slate-500 font-black text-[10px] uppercase tracking-[0.15em]">Billing Cycle</Label>
                      <select
                        id="billingCycle"
                        name="billingCycle"
            ...
            <div className="space-y-10">
              <Card className="rounded-[32px] border-none shadow-xl bg-slate-900 text-white p-2">
                ...
                <CardContent className="px-8 py-8 space-y-8">
                   <div className="space-y-3">
                      <Label className="text-slate-400 font-black text-[10px] uppercase tracking-[0.15em]">Effective Start Date</Label>
                      <Input
                        type="date"
                        name="startDate"
                        required
                   ...
                   <div className="space-y-3">
                      <Label className="text-slate-400 font-black text-[10px] uppercase tracking-[0.15em]">Expiry Date</Label>
                      <Input
                        type="date"
                        name="endDate"
                        required
              ...
              <Card className="rounded-[32px] border-none shadow-xl bg-white p-2 border-2 border-primary/10">
                ...
                <CardContent className="px-8 py-8 space-y-8">
                   <div className="space-y-3">
                      <Label className="text-slate-500 font-black text-[10px] uppercase tracking-[0.15em]">Total Amount (INR)</Label>
                      <Input
                        type="number"
                        name="contractValue"
                   ...
                   <div className="space-y-3">
                      <Label className="text-slate-500 font-black text-[10px] uppercase tracking-[0.15em]">PM Visits Per Year</Label>
                      <Input
                        type="number"
              ...
              <div className="p-8 bg-blue-50/50 rounded-[32px] border-2 border-blue-100 flex items-start gap-4">
                 ...
                 <div className="space-y-2">
                    <p className="text-sm font-black text-blue-900 uppercase tracking-tighter">Modification Notice</p>
                    <p className="text-xs text-blue-700 font-medium leading-relaxed">
                       Significant changes to billing cycles or SLA terms should be documented in the amendment notes or communicated to the customer.
                    </p>
                 </div>
              </div>
            </div>
