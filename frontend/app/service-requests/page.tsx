...
import { ServiceRequest, ServiceRequestStatus, ServiceRequestSource } from "@/types/field-service";
...
export default function ServiceRequestsPage() {
  ...
  const getSourceIcon = (source: ServiceRequestSource) => {
    switch (source) {
      case ServiceRequestSource.PHONE:
        return <Phone className="h-3.5 w-3.5" />;
      case ServiceRequestSource.EMAIL:
        return <Mail className="h-3.5 w-3.5" />;
      case ServiceRequestSource.PORTAL:
        return <Globe className="h-3.5 w-3.5" />;
      case ServiceRequestSource.WHATSAPP:
  ...
  const getStatusBadge = (status: ServiceRequestStatus) => {
    switch (status) {
      case ServiceRequestStatus.OPEN:
        return (
          <Badge className="bg-blue-50 text-blue-700 border-blue-100 uppercase text-[9px] font-black tracking-widest px-3">
            New
          </Badge>
        );
      case ServiceRequestStatus.ACKNOWLEDGED:
        return (
          <Badge className="bg-indigo-50 text-indigo-700 border-indigo-100 uppercase text-[9px] font-black tracking-widest px-3">
            Acked
          </Badge>
        );
      case ServiceRequestStatus.RESOLVED:
        return (
          <Badge className="bg-emerald-50 text-emerald-700 border-emerald-100 uppercase text-[9px] font-black tracking-widest px-3">
            Resolved
          </Badge>
        );
      case ServiceRequestStatus.CLOSED:
        return (
          <Badge className="bg-slate-100 text-slate-500 border-slate-200 uppercase text-[9px] font-black tracking-widest px-3">
            Closed
          </Badge>
        );
      case ServiceRequestStatus.CANCELLED:
        return (
          <Badge className="bg-rose-50 text-rose-700 border-rose-100 uppercase text-[9px] font-black tracking-widest px-3">
            Cancelled
          </Badge>
        );
      default:
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        {loading ? (
          ...
          <div className="bg-white rounded-[4rem] p-32 text-center shadow-2xl border-2 border-dashed border-slate-100 flex flex-col items-center animate-in zoom-in-95 duration-500">
            ...
            <p className="text-slate-400 font-bold max-w-sm mx-auto mb-10 text-lg leading-relaxed italic opacity-80">
              All service requests have been processed or no entries match your search parameters.
            </p>
            <Button
              onClick={() => router.push("/service-requests/new")}
              className="rounded-[2.5rem] h-16 px-12 font-black shadow-2xl shadow-indigo-600/30 bg-indigo-600"
            >
          ...
          <div className="grid grid-cols-1 gap-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
            {filteredRequests
              .sort((a, b) => b.srNumber.localeCompare(a.srNumber))
              .map((sr) => (
                <Card
                  key={sr.id}
                      ...
                      <div className="flex-shrink-0 flex items-center gap-6">
                        <div className="h-16 w-16 bg-slate-50 rounded-3xl flex flex-col items-center justify-center border-2 border-slate-100 group-hover:border-indigo-600/20 transition-colors">
                          ...
                          <span className="text-sm font-black text-slate-900 tracking-tight uppercase">
                            {sr.srNumber.split("-")[1] || sr.srNumber}
                          </span>
                        </div>
                        <div>
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            <Badge
                              variant="outline"
                              className={`rounded-xl text-[9px] font-black uppercase tracking-widest px-2.5 py-1 ${sr.priority === "HIGH" || sr.priority === "CRITICAL" ? "text-rose-600 border-rose-100 bg-rose-50" : "text-slate-500 border-slate-200"}`}
                            >
                              {sr.priority}
                            </Badge>
                          </div>
                        ...
                        {sr.workOrderId && (
                          <div className="hidden lg:flex flex-col items-end px-6 border-r border-slate-100">
                            ...
                            <p className="text-xs font-black text-indigo-600 uppercase tracking-tighter">
                              {sr.workOrderId.split("-")[1] || "Released"}
                            </p>
                          </div>
                        )}
                        <Button
                          variant="ghost"
                          size="icon"
                          className="h-14 w-14 rounded-2xl group-hover:bg-slate-900 group-hover:text-white transition-all shadow-none"
                        >
