...
export default function ServiceRequestDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const dispatchWorkOrder = () => {
    router.push(
      `/work-orders/new?srId=${id}&accountId=${sr?.accountId}&assetId=${sr?.assetId || ""}`
    );
  };
  ...
  const getStatusBadge = (status: ServiceRequestStatus) => {
    switch (status) {
      case ServiceRequestStatus.OPEN:
        return (
          <Badge className="bg-blue-50 text-blue-700 border-blue-100 uppercase text-[10px] font-black tracking-widest px-4">
            Unassigned Log
          </Badge>
        );
      case ServiceRequestStatus.ACKNOWLEDGED:
        return (
          <Badge className="bg-indigo-50 text-indigo-700 border-indigo-100 uppercase text-[10px] font-black tracking-widest px-4">
            Awaiting Dispatch
          </Badge>
        );
      case ServiceRequestStatus.RESOLVED:
        return (
          <Badge className="bg-emerald-50 text-emerald-700 border-emerald-100 uppercase text-[10px] font-black tracking-widest px-4">
            Resolved
          </Badge>
        );
      case ServiceRequestStatus.CLOSED:
        return (
          <Badge className="bg-slate-100 text-slate-500 border-slate-200 uppercase text-[10px] font-black tracking-widest px-4">
            Archived
          </Badge>
        );
      default:
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between py-5">
            <div className="flex items-center gap-6">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500 font-bold uppercase tracking-widest">
                  Logged via {sr.source} • {new Date(sr.createdAt).toLocaleString()}
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3">
              ...
              {(sr.status === ServiceRequestStatus.OPEN ||
                sr.status === ServiceRequestStatus.ACKNOWLEDGED) &&
                !sr.workOrderId && (
                  <Button
              ...
              <Card className="rounded-[2.5rem] border-none shadow-xl bg-white p-2 group overflow-hidden">
                <CardContent className="p-8 flex items-center gap-6">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <h4 className="text-xl font-black text-slate-900 tracking-tight leading-tight mb-1 truncate">
                      {account?.accountName}
                    </h4>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <CardDescription className="text-slate-400 font-medium">
                      Categorized complaint data as logged by the channel.
                    </CardDescription>
                  </div>
                </div>
              </CardHeader>
              <CardContent className="px-10 pb-10 pt-6 space-y-10">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 pt-6 border-t border-slate-50">
                  ...
                  <div className="space-y-1">
                    ...
                    <p
                      className={`font-black flex items-center gap-2 ${sr.priority === "HIGH" ? "text-rose-600" : "text-slate-900"}`}
                    >
                  ...
                  <div className="flex items-center gap-6">
                    ...
                    <div>
                      ...
                      <h4 className="text-2xl font-black tracking-tight">
                        {workOrder ? workOrder.woNumber : "Dispatch Active"}
                      </h4>
                      <p className="text-sm font-bold opacity-80">
                        This ticket is currently being handled in the field.
                      </p>
                    </div>
                  </div>
                  <Button
                    onClick={() => router.push(`/work-orders/${sr.workOrderId}`)}
                    className="rounded-[2rem] h-14 px-10 bg-white text-indigo-600 font-black hover:bg-slate-100 shadow-2xl"
                  >
                    Go to Operation Hub
          ...
          <div className="space-y-10">
            <Card className="rounded-[3rem] border-none shadow-xl bg-slate-900 text-white p-2">
              ...
              <CardContent className="px-8 py-8 space-y-8">
                <div className="relative pl-8 border-l-2 border-white/5 space-y-10">
                  ...
                  <div className="relative">
                    <div
                      className={`absolute -left-[41px] top-0 h-6 w-6 rounded-full border-4 border-slate-900 flex items-center justify-center ${sr.acknowledgedAt ? "bg-emerald-500" : "bg-slate-700"}`}
                    >
                    ...
                    <div>
                      ...
                      <p className="text-sm font-bold">
                        {sr.acknowledgedAt
                          ? new Date(sr.acknowledgedAt).toLocaleString()
                  ...
                  <div className="relative">
                    <div
                      className={`absolute -left-[41px] top-0 h-6 w-6 rounded-full border-4 border-slate-900 flex items-center justify-center ${sr.workOrderId ? "bg-emerald-500" : "bg-slate-700 animate-pulse"}`}
                    >
                    ...
                    <div>
                      ...
                      <p className="text-sm font-bold">
                        {sr.workOrderId ? "Action Released" : "Awaiting Coordination"}
                      </p>
                    </div>
                  </div>
            ...
            <div className="p-8 bg-amber-50 rounded-[3rem] border-2 border-amber-100 flex flex-col gap-4">
              ...
              <p className="text-[11px] text-amber-700 font-medium leading-relaxed">
                SLA targets are binding based on the customer contract. Failure to acknowledge
                within the first 4 hours impacts system performance metrics.
              </p>
            </div>
          </div>
        </div>
