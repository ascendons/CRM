    ...
    <div className="p-6 max-w-4xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex gap-2">
          <a href="/hr/onboarding/templates" className="border px-4 py-2 rounded-lg text-sm">
            Templates
          </a>
        </div>
      </div>
      ...
      <div className="space-y-3">
        {instances.map((inst: any) => (
          <a
            key={inst.instanceId}
            href={`/hr/onboarding/${inst.instanceId}`}
            className="block bg-white border rounded-xl p-4 hover:border-primary transition-colors"
          >
            <div className="flex items-center justify-between mb-2">
              <div>
                <p className="font-semibold">Employee: {inst.employeeId}</p>
                <p className="text-sm text-slate-500">
                  Mentor: {inst.mentorId || "N/A"} · Started: {inst.startDate}
                </p>
              </div>
