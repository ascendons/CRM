  ...
  return (
    <div className="p-6 max-w-3xl mx-auto">
      <div className="mb-6">
        ...
        <p className="text-slate-500">Mentor: {instance.mentorId || "N/A"} · Started: {instance.startDate}</p>
      </div>
      ...
      <div className="bg-white border rounded-xl p-4 mb-4">
        ...
        <div className="w-full bg-slate-100 rounded-full h-3">
          <div className="bg-primary h-3 rounded-full transition-all" style={{ width: `${instance.progress}%` }} />
        </div>
      </div>

      <div className="space-y-2">
        {(instance.tasks || []).map((task: any, i: number) => (
          <div key={i} className={`bg-white border rounded-xl p-4 flex items-center gap-3 ${task.status === "DONE" ? "opacity-60" : ""}`}>
            <input
              type="checkbox"
            ...
            <div className="flex-1">
              <p className={`font-medium ${task.status === "DONE" ? "line-through" : ""}`}>{task.taskTitle}</p>
              {task.description && <p className="text-sm text-slate-500">{task.description}</p>}
              <p className="text-xs text-slate-400">Assignee: {task.assigneeTo} · Due: Day {task.dueDaysFromStart}</p>
            </div>
            {task.status === "DONE" && <span className="text-green-600 text-sm font-medium">Done</span>}
            {task.isRequired && task.status !== "DONE" && <span className="text-xs text-red-500">Required</span>}
          </div>
        ))}
      </div>
    </div>
