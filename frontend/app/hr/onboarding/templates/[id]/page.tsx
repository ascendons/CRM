      ...
      <div className="bg-white border rounded-xl p-6 space-y-4">
        ...
        <div>
          <div className="flex items-center justify-between mb-2">
            ...
            <button onClick={addTask} className="text-primary text-sm underline">
              + Add Task
            </button>
          </div>
          {(form.tasks || []).map((task: any, i: number) => (
            <div key={i} className="border rounded-lg p-3 mb-2 space-y-2">
              <input
                ...
                onChange={(e) => {
                  const tasks = [...form.tasks];
                  tasks[i].taskTitle = e.target.value;
                  setForm({ ...form, tasks });
                }}
              />
              <input
              ...
              <div className="flex gap-2">
                <select
                  ...
                  value={task.assigneeTo}
                  onChange={(e) => {
                    const tasks = [...form.tasks];
                    tasks[i].assigneeTo = e.target.value;
                    setForm({ ...form, tasks });
                  }}
                >
                  {["SELF", "MANAGER", "HR", "IT"].map((a) => (
                ...
                <input
                  ...
                  onChange={(e) => {
                    const tasks = [...form.tasks];
                    tasks[i].dueDaysFromStart = parseInt(e.target.value);
                    setForm({ ...form, tasks });
                  }}
                />
                <label className="flex items-center gap-1 text-sm">
                  <input
                    ...
                    onChange={(e) => {
                      const tasks = [...form.tasks];
                      tasks[i].isRequired = e.target.checked;
                      setForm({ ...form, tasks });
                    }}
                  />
                  Required
                </label>
                <button onClick={() => removeTask(i)} className="text-red-500 text-sm">
                  Remove
                </button>
              </div>
            </div>
        ...
        <button onClick={handleSave} className="bg-primary text-white px-6 py-2 rounded-lg">
          Save Template
        </button>
      </div>
    </div>
