...
import { projectsService, Project, ProjectTask, TaskStatus } from "@/lib/projects";
import { showToast } from "@/lib/toast";
import {
  ArrowLeft, Users, Calendar, DollarSign, Tag, Edit, Trash2,
  LayoutGrid, List, BarChart2, PlusCircle
} from "lucide-react";
...
export default function ProjectDetailPage() {
  ...
  const tasksByStatus = (Object.keys(STATUS_LABELS) as TaskStatus[]).reduce((acc, s) => {
    ...
    <div className="p-6">
      ...
      <div className="flex items-start justify-between mb-6">
        <div className="flex items-center gap-3">
          <Link href="/projects" className="text-gray-500 hover:text-gray-700"><ArrowLeft className="w-5 h-5" /></Link>
          <div>
        ...
        <div className="flex gap-2">
          <button onClick={handleDelete} className="flex items-center gap-1 text-red-600 border border-red-200 px-3 py-1.5 rounded-lg text-sm hover:bg-red-50">
          ...
          <div className="md:col-span-2 space-y-4">
            ...
            <div className="bg-white rounded-xl border border-gray-200 p-5">
              <div className="flex items-center justify-between mb-3">
                ...
                <Link
                  href={`/project-tasks?projectId=${project.projectId}`}
              ...
              <div className="bg-gray-50 rounded-xl p-3">
                ...
                <div className="space-y-2">
                  {tasksByStatus[status]?.map(task => (
                    <div key={task.taskId} className="bg-white rounded-lg border border-gray-200 p-3 hover:shadow-sm transition-shadow">
                      <Link href={`/projects/${id}/tasks/${task.taskId}`}>
                        <p className="text-sm font-medium text-gray-800 mb-1">{task.title}</p>
                        {task.dueDate && (
                          <p className="text-xs text-gray-400">{new Date(task.dueDate).toLocaleDateString("en-IN")}</p>
                        )}
                      </Link>
            ...
            <tbody>
              {tasks.map(task => (
                <tr key={task.taskId} className="border-b border-gray-50 hover:bg-gray-50">
                  <td className="p-4">
                    <Link href={`/projects/${id}/tasks/${task.taskId}`} className="text-blue-600 hover:underline">
                      {task.title}
                    </Link>
                  </td>
                  <td className="p-4">
              ...
              {tasks.length === 0 && (
                <tr><td colSpan={4} className="p-8 text-center text-gray-400">No tasks yet</td></tr>
              )}
            </tbody>
          </table>
        </div>
      ...
      {activeTab === "gantt" && (
        <div className="bg-white rounded-xl border border-gray-200 p-5">
          ...
          {tasks.filter(t => t.dueDate).length === 0 ? (
            <p className="text-gray-400 text-sm">No tasks with due dates to display on Gantt chart.</p>
          ) : (
            <div className="space-y-3">
              {tasks.filter(t => t.dueDate).map(task => {
                const start = project.startDate ? new Date(project.startDate) : new Date();
                const end = project.dueDate ? new Date(project.dueDate) : new Date(Date.now() + 30 * 24 * 60 * 60 * 1000);
                const taskDue = new Date(task.dueDate!);
                const total = end.getTime() - start.getTime();
                const taskPos = Math.max(0, Math.min(100, ((taskDue.getTime() - start.getTime()) / total) * 100));
                return (
