...
export default function NewLandingPagePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      showToast("Failed to create landing page", "error");
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="p-6 max-w-2xl mx-auto">
      <div className="flex items-center gap-3 mb-6">
        <Link href="/marketing/landing-pages" className="text-gray-500 hover:text-gray-700"><ArrowLeft className="w-5 h-5" /></Link>
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-xl border border-gray-200 p-6 space-y-5">
        ...
        <div>
          ...
          <div className="flex items-center border border-gray-300 rounded-lg overflow-hidden focus-within:ring-2 focus-within:ring-blue-500">
            <span className="bg-gray-50 px-3 py-2 text-sm text-gray-400 border-r border-gray-300">/lp/</span>
            <input type="text" value={form.slug} onChange={e => setForm({ ...form, slug: e.target.value })}
              className="flex-1 px-3 py-2 text-sm focus:outline-none" placeholder="get-started" />
          </div>
        </div>

        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">Hero Text</label>
          <textarea rows={3} value={form.heroText} onChange={e => setForm({ ...form, heroText: e.target.value })}
            className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
        ...
        <div className="flex justify-end gap-3 pt-2">
          <Link href="/marketing/landing-pages" className="px-4 py-2 border border-gray-300 rounded-lg text-sm text-gray-700 hover:bg-gray-50">Cancel</Link>
