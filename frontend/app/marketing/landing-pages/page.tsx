  ...
  return (
    <div className="p-6">
      <div className="flex items-center justify-between mb-6">
        ...
        <Link href="/marketing/landing-pages/new" className="flex items-center gap-2 bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 text-sm">
      ...
      {pages.length === 0 ? (
        <div className="text-center py-16">
          ...
          <Link href="/marketing/landing-pages/new" className="text-blue-600 text-sm mt-2 inline-block">Create your first landing page</Link>
        </div>
      ) : (
          ...
          {pages.map(page => (
            <div key={page.pageId} className="bg-white rounded-xl border border-gray-200 p-5">
              <div className="flex items-start justify-between mb-3">
                ...
                <button onClick={() => handleDelete(page.pageId)} className="text-gray-400 hover:text-red-500">
                  <Trash2 className="w-4 h-4" />
                </button>
              </div>
              {page.heroText && <p className="text-sm text-gray-500 line-clamp-2 mb-3">{page.heroText}</p>}
              <div className="flex items-center justify-between mt-4">
                ...
                <div className="flex gap-2">
                  {!page.published && (
                    <button onClick={() => handlePublish(page.pageId)} className="text-xs text-blue-600 hover:text-blue-700">Publish</button>
                  )}
                  {page.published && (
                    <Link href={`/lp/${page.slug}`} target="_blank" className="flex items-center gap-1 text-xs text-gray-500 hover:text-gray-700">
