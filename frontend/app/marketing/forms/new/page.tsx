...
const FIELD_TYPES: FormFieldType[] = ["TEXT", "EMAIL", "PHONE", "NUMBER", "DROPDOWN", "CHECKBOX", "TEXTAREA", "DATE"];
      ...
      <form onSubmit={handleSubmit}>
            ...
            <div className="col-span-3 space-y-4">
              ...
              <div className="bg-white rounded-xl border border-gray-200 p-5">
                ...
                {form.fields?.length === 0 && (
                  <p className="text-gray-400 text-sm text-center py-4">No fields yet. Click Add Field to start.</p>
                )}
                <div className="space-y-3">
                  {form.fields?.map((field, i) => (
                    <div key={field.fieldId} className="flex items-start gap-3 p-3 border border-gray-200 rounded-lg">
              ...
              <div className="bg-white rounded-xl border border-gray-200 p-5 space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <label className="flex items-center gap-2 text-sm text-gray-600">
                    <input type="checkbox" checked={form.submitAction?.createLead}
                      onChange={e => setForm({ ...form, submitAction: { ...form.submitAction, createLead: e.target.checked } })} />
                    Create Lead
                  </label>
                  <label className="flex items-center gap-2 text-sm text-gray-600">
                    <input type="checkbox" checked={form.submitAction?.createContact}
                      onChange={e => setForm({ ...form, submitAction: { ...form.submitAction, createContact: e.target.checked } })} />
                    Create Contact
                  </label>
                </div>
              </div>
        ...
        {activeTab === "preview" && (
          <div className="bg-white rounded-xl border border-gray-200 p-8 max-w-md mx-auto">
            <h2 className="text-xl font-bold mb-6" style={{ color: form.themeColor }}>{form.name || "Form Preview"}</h2>
            <div className="space-y-4">
              {form.fields?.map(field => (
                <div key={field.fieldId}>
                  ...
                  {field.type === "TEXTAREA" ? (
                    <textarea rows={3} disabled className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm bg-gray-50" />
                  ) : field.type === "CHECKBOX" ? (
                    ...
                    <input type={field.type.toLowerCase()} disabled className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm bg-gray-50" />
                  )}
                </div>
              ))}
              {form.fields?.length === 0 && <p className="text-gray-400 text-center py-8">Add fields to see preview</p>}
              <button disabled className="w-full py-2 rounded-lg text-white text-sm font-medium" style={{ backgroundColor: form.themeColor }}>
                Submit
              </button>
            </div>
          </div>
        ...
        {activeTab === "embed" && (
          <div className="bg-white rounded-xl border border-gray-200 p-6">
            ...
            <p className="text-sm text-gray-500 mb-4">Save the form first, then copy the embed code from the forms list page.</p>
        ...
        <div className="flex justify-end gap-3 mt-6">
          <Link href="/marketing/forms" className="px-4 py-2 border border-gray-300 rounded-lg text-sm text-gray-700 hover:bg-gray-50">Cancel</Link>
          <button type="submit" disabled={saving} className="flex items-center gap-2 bg-blue-600 text-white px-4 py-2 rounded-lg text-sm hover:bg-blue-700 disabled:opacity-50">
