...
export default function FileDetailPage() {
  ...
  useEffect(() => {
    driveApi
      .downloadFile(id)
      .then((res) => setDownloadUrl(res.data))
      .catch(() => {});
    driveApi
      .getVersions(id)
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto">
      ...
      <div className="bg-white border rounded-xl p-4 mb-4">
        {isImage && downloadUrl && (
          <img src={downloadUrl} alt={file?.name} className="max-w-full max-h-96 mx-auto rounded" />
        )}
        {isPdf && downloadUrl && (
          <iframe src={downloadUrl} className="w-full h-[600px] rounded" title={file?.name} />
        )}
        {!isImage && !isPdf && (
      ...
      {downloadUrl && (
        <a
          href={downloadUrl}
          download={file?.name}
          className="inline-flex items-center gap-2 bg-primary text-white px-4 py-2 rounded-lg mb-6"
        >
      ...
      <div>
        ...
        {versions.map((v: any) => (
          <div
            key={v.versionId}
            className="bg-white border rounded-lg p-3 mb-2 flex items-center justify-between"
          >
            <div>
              <p className="font-medium text-sm">Version {v.versionNumber}</p>
              <p className="text-xs text-slate-500">
                Uploaded by {v.uploadedBy} · {v.uploadedAt?.substring(0, 10)}
              </p>
            </div>
            <span className="text-xs text-slate-400">
              {v.sizeBytes ? `${(v.sizeBytes / 1024).toFixed(1)} KB` : ""}
            </span>
          </div>
        ))}
