...
export default function DrivePage() {
  ...
  const [breadcrumb, setBreadcrumb] = useState<{ id?: string; name: string }[]>([{ name: "Drive" }]);
  ...
  async function handleUpload(fileList: FileList | null) {
    ...
    for (const f of Array.from(fileList)) {
      ...
      } catch { showToast(`Failed to upload ${f.name}`, "error"); }
    }
    load();
  }
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto">
      ...
      <div className="flex items-center gap-1 mb-4 text-sm">
        {breadcrumb.map((b, i) => (
          <span key={i} className="flex items-center gap-1">
            ...
            <button onClick={() => navigateTo(i)} className={i === breadcrumb.length - 1 ? "font-semibold text-primary" : "text-slate-500 hover:text-primary"}>{b.name}</button>
          </span>
        ))}
      </div>
      ...
      <div className="flex items-center gap-3 mb-4">
        <input className="border rounded-lg px-3 py-2 text-sm" placeholder="New folder name" value={newFolder} onChange={e => setNewFolder(e.target.value)} />
        <button onClick={handleCreateFolder} className="bg-slate-100 border px-3 py-2 rounded-lg text-sm">Create Folder</button>
        <button onClick={() => fileRef.current?.click()} className="bg-primary text-white px-4 py-2 rounded-lg text-sm flex items-center gap-2">
      ...
      {folders.length > 0 && (
        <div className="mb-4">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {folders.map((f: any) => (
              <button key={f.folderId} onClick={() => navigate(f)} className="bg-white border rounded-xl p-4 flex items-center gap-3 hover:border-primary transition-colors text-left">
            ...
            {files.map((f: any) => (
              <div key={f.fileId} className="bg-white border rounded-xl p-4 group relative">
                <a href={`/drive/files/${f.fileId}`} className="flex flex-col items-center gap-2">
                  <span className="material-symbols-outlined text-4xl text-blue-500">{fileIcon(f.mimeType)}</span>
                  <p className="text-sm font-medium text-center truncate w-full">{f.name}</p>
                  <p className="text-xs text-slate-400">{f.sizeBytes ? `${(f.sizeBytes / 1024).toFixed(1)} KB` : ""}</p>
                </a>
                <button onClick={() => handleDelete(f.fileId)} className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 text-red-500">
