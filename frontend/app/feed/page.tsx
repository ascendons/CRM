...
export default function FeedPage() {
  ...
  async function handlePost() {
    ...
    if (tab === "POLL") {
      data.pollOptions = compose.pollOptions
        .filter((o) => o.trim())
        .map((o) => ({ option: o, voteCount: 0 }));
    }
    try {
  ...
  async function handleVote(postId: string, optionIndex: number) {
    try {
      await feedApi.vote(postId, optionIndex);
      showToast.success("Voted");
      load(0);
    } catch {
      showToast.error("Already voted");
    }
  }
    ...
    <div className="p-6 max-w-2xl mx-auto">
        ...
        <div className="flex gap-2 mb-3">
          {(["POST", "ANNOUNCEMENT", "POLL"] as const).map((t) => (
            <button
              ...
              onClick={() => {
                setTab(t);
                setCompose((c) => ({ ...c, type: t }));
              }}
              className={`px-3 py-1 rounded-lg text-sm font-medium ${tab === t ? "bg-primary text-white" : "bg-slate-100"}`}
            >
              {t}
            </button>
          ))}
          ...
          <div className="mt-2 space-y-2">
            {compose.pollOptions.map((opt, i) => (
              <input
                ...
                value={opt}
                onChange={(e) => {
                  const opts = [...compose.pollOptions];
                  opts[i] = e.target.value;
                  setCompose({ ...compose, pollOptions: opts });
                }}
              />
            ))}
            <button
              className="text-primary text-sm underline"
              onClick={() => setCompose({ ...compose, pollOptions: [...compose.pollOptions, ""] })}
            >
              + Add option
            </button>
          </div>
      ...
      <div className="space-y-4">
        {posts.map((post: any) => (
          <div
            key={post.postId}
            className={`bg-white border rounded-xl p-4 ${post.isPinned ? "border-amber-300 bg-amber-50/20" : ""}`}
          >
            {post.isPinned && (
            ...
            <div className="flex items-center gap-2 mb-2">
              ...
              <div>
                ...
                <p className="text-xs text-slate-400">
                  {post.createdAt?.substring(0, 16).replace("T", " ")}
                </p>
              </div>
              <span
                className={`ml-auto px-2 py-0.5 rounded text-xs font-medium ${post.type === "ANNOUNCEMENT" ? "bg-red-100 text-red-700" : post.type === "POLL" ? "bg-purple-100 text-purple-700" : "bg-slate-100 text-slate-600"}`}
              >
                {post.type}
              </span>
            </div>
              ...
              <div className="space-y-2 mb-3">
                {post.pollOptions.map((opt: any, i: number) => {
                  const total = post.pollOptions.reduce(
                    (s: number, o: any) => s + (o.voteCount || 0),
                    0
                  );
                  ...
                  return (
                    <button
                      key={i}
                      onClick={() => handleVote(post.postId, i)}
                      className="w-full text-left border rounded-lg p-2 hover:border-primary"
                    >
                      ...
                      <div className="bg-slate-100 rounded h-1.5">
                        <div className="bg-primary h-1.5 rounded" style={{ width: `${pct}%` }} />
                      </div>
                    </button>
                  );
                })}
      ...
      {hasMore && (
        <button
          onClick={() => load(page + 1)}
          className="w-full mt-4 py-2 border rounded-lg text-sm text-slate-600 hover:bg-slate-50"
        >
          Load more
        </button>
      )}
