...
export default function AssetCategoriesPage() {
  ...
  const getCategoryIcon = (type: AssetCategoryType) => {
    switch (type) {
      case AssetCategoryType.EQUIPMENT:
        return <Wrench className="h-4 w-4" />;
      case AssetCategoryType.SPARE_PART:
        return <Package className="h-4 w-4" />;
      case AssetCategoryType.CONSUMABLE:
        ...
        {loading ? (
          ...
          <Card className="rounded-[2.5rem] border-dashed border-2 bg-slate-50/50 p-20 text-center flex flex-col items-center">
            ...
            <p className="text-slate-500 font-medium max-w-sm mb-8">
              Asset categories help you organize equipment and define standard maintenance
              checklists.
            </p>
            <Button
              onClick={() => handleOpenModal()}
              className="rounded-2xl px-8 py-6 h-auto font-bold shadow-xl shadow-primary/20"
            >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredCategories.map((category) => (
              <Card
                key={category.id}
                className="group rounded-[2.5rem] border-slate-200 bg-white shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 overflow-hidden cursor-default border-2 hover:border-primary/20"
              >
                  ...
                  <div className="space-y-4 pt-6 border-t border-slate-100">
                    <div className="flex items-center justify-between text-[11px] font-bold uppercase tracking-widest text-slate-400">
                      ...
                      <span className="text-slate-900">
                        {category.maintenanceIntervalDays} Days
                      </span>
                    </div>
                      ...
                      {category.requiredSkills.length > 0 ? (
                        category.requiredSkills.map((skill) => (
                          <Badge
                            key={skill}
                            variant="secondary"
                            className="bg-slate-50 text-slate-600 border-slate-100 rounded-lg text-[10px] font-bold px-2.5 py-1 uppercase tracking-tight"
                          >
                            {skill}
                          </Badge>
                        ))
              ...
              <div className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="space-y-2">
                    ...
                    <select
                      ...
                      value={formData.type}
                      onChange={(e) =>
                        setFormData({ ...formData, type: e.target.value as AssetCategoryType })
                      }
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        setFormData({
                          ...formData,
                          maintenanceIntervalDays: parseInt(e.target.value),
                        })
                      }
                  ...
                  <div className="space-y-2">
                    <Label
                      htmlFor="skills"
                      className="text-[11px] font-black uppercase tracking-widest text-slate-500 font-bold"
                    >
