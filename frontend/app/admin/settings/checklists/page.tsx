...
import { Checklist, ChecklistItem, WorkOrderType, AssetCategory } from "@/types/field-service";
...
export default function ChecklistManagementPage() {
  ...
  const handleDelete = async (id: string) => {
    if (
      !confirm(
        "Confirm decommissioning of this protocol? This will not affect existing work records."
      )
    )
      return;
    try {
          ...
          {filteredTemplates.length === 0 ? (
            <div className="bg-white rounded-[3rem] p-32 text-center shadow-xl border-2 border-dashed border-slate-100 flex flex-col items-center">
              ...
              <p className="text-slate-500 font-medium max-w-sm mx-auto mb-10">
                Define your service standards by creating checklist templates for different
                equipment categories.
              </p>
              <Button
                onClick={handleCreate}
                className="rounded-[2rem] h-14 px-10 font-black shadow-2xl shadow-primary/20"
              >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {filteredTemplates.map((t) => (
                <Card
                  key={t.id}
                  className="rounded-[2.5rem] border-none shadow-lg hover:shadow-2xl transition-all duration-300 bg-white group overflow-hidden"
                >
                    ...
                    <div className="space-y-4">
                      <div>
                        ...
                        <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">
                          For{" "}
                          {assetCategories.find((c) => c.id === t.assetCategoryId)?.name ||
                            "All Equipment"}
                        </p>
                      </div>

                      <div className="flex items-center gap-3 pt-4 border-t border-slate-50">
                        <Badge className="bg-slate-900 text-white rounded-lg text-[10px] font-black p-2">
                          {t.jobType}
                        </Badge>
                        <Badge
                          variant="outline"
                          className="rounded-lg text-[10px] font-black p-2 border-slate-200 text-slate-500"
                        >
                          {t.items?.length || 0} Steps
                        </Badge>
                      </div>
                    </div>
                  </CardContent>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/40 backdrop-blur-md animate-in fade-in duration-300">
          <div className="bg-white rounded-[3rem] w-full max-w-4xl max-h-[90vh] overflow-hidden shadow-2xl animate-in zoom-in-95 duration-300 border-2 border-white flex flex-col">
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">
                    {editingTemplate.id ? "Modify Existing Standard" : "Establish New Standard"}
                  </p>
                </div>
              </div>
              <button
            ...
            <form
              ...
              className="flex-1 overflow-y-auto custom-scrollbar p-10 pt-8 space-y-10"
            >
              <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                ...
                <div className="space-y-3">
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setEditingTemplate({ ...editingTemplate, assetCategoryId: e.target.value })
                    }
                ...
                <div className="space-y-3">
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setEditingTemplate({ ...editingTemplate, jobType: e.target.value as any })
                    }
                    className="w-full h-12 rounded-xl border-2 border-slate-50 bg-slate-50/50 px-4 text-sm font-bold focus:outline-none focus:border-primary appearance-none transition-all"
                  >
              ...
              <div className="space-y-6">
                <div className="flex items-center justify-between px-1">
                  ...
                  <Button
                    ...
                    className="rounded-xl h-9 px-4 font-black text-[10px] uppercase border-2 border-slate-100 hover:bg-slate-50"
                  >
                ...
                <div className="space-y-4">
                  {editingTemplate.items?.map((item, idx) => (
                    <div
                      key={idx}
                      className="bg-slate-50 rounded-2xl p-6 border-2 border-slate-100 flex items-start gap-4 animate-in slide-in-from-right-4"
                    >
                      ...
                      <div className="flex-1 grid grid-cols-1 md:grid-cols-12 gap-6">
                        ...
                        <div className="md:col-span-3 space-y-2">
                          ...
                          <select
                            ...
                            onChange={(e) =>
                              handleItemChange(idx, { inputType: e.target.value as any })
                            }
                        ...
                        <div className="md:col-span-2 space-y-2 pt-8">
                          <label className="flex items-center gap-3 cursor-pointer group">
                            <div
                              className={`h-5 w-5 rounded-md border-2 transition-all flex items-center justify-center ${item.isMandatory ? "bg-primary border-primary" : "border-slate-200"}`}
                            >
                              {item.isMandatory && (
                            ...
                            <input
                              ...
                              onChange={(e) =>
                                handleItemChange(idx, { isMandatory: e.target.checked })
                              }
                            />
                  ...
                  {(!editingTemplate.items || editingTemplate.items.length === 0) && (
                    <div className="p-12 text-center border-2 border-dashed border-slate-200 rounded-[2rem]">
                      <p className="text-[10px] font-black text-slate-300 uppercase tracking-widest italic">
                        No inspection steps defined yet.
                      </p>
                    </div>
                  )}
                </div>
            ...
            <div className="p-10 border-t border-slate-50 flex items-center justify-end gap-4 bg-slate-50/50">
              <Button
                variant="ghost"
                onClick={() => setEditingTemplate(null)}
                className="rounded-2xl h-12 px-8 font-black uppercase text-xs tracking-widest text-slate-400"
              >
                Discard Changes
              </Button>
              <Button
