...
export default function EditWorkOrderPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const loadInitialData = async () => {
    try {
      ...
      setEngineers(usersData.filter(u => u.roleName?.includes("Engineer") || u.roleName?.includes("Technician")));
    ...
    return (
      <div className="flex items-center justify-center bg-slate-50 min-h-[calc(100vh-4rem)]">
        <div className="text-center space-y-4">
           ...
           <p className="text-slate-500 font-medium tracking-tight">Accessing engineering record...</p>
        </div>
      </div>
    );
  }
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between py-5">
            <div className="flex items-center gap-5">
              ...
              <div>
                <h1 className="text-2xl font-black text-slate-900 tracking-tight">Modify Work Order</h1>
                <p className="text-xs text-slate-500 font-bold uppercase tracking-widest">Update Diagnostics & Workflow</p>
              </div>
            </div>
           ...
           <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
                 ...
                 <Card className="rounded-[2.5rem] border-none shadow-xl bg-white p-2">
                    <CardHeader className="px-8 pt-8 pb-4">
                       <div className="flex items-center gap-3">
                          ...
                          <div>
                             <CardTitle className="text-xl font-black tracking-tight text-slate-900">Professional Diagnosis</CardTitle>
                             <CardDescription className="text-slate-400 font-medium">Record the technical cause and observed issues.</CardDescription>
                          </div>
                       </div>
                    </CardHeader>
                    <CardContent className="px-8 pb-8 pt-4 space-y-6">
                       <div className="space-y-3">
                          <Label className="text-[10px] font-black text-slate-500 uppercase tracking-widest px-1">Chief Symptoms (Updated)</Label>
                          <textarea
                            name="symptoms"
                            rows={3}
                       ...
                       <div className="space-y-3">
                          <Label className="text-[10px] font-black text-slate-500 uppercase tracking-widest px-1">Technician Diagnosis *</Label>
                          <textarea
                            name="diagnosis"
                            required
                            rows={4}
                 ...
                 <Card className="rounded-[2.5rem] border-none shadow-xl bg-white p-2">
                    <CardHeader className="px-8 pt-8 pb-4">
                       <div className="flex items-center gap-3">
                          ...
                          <div>
                             <CardTitle className="text-xl font-black tracking-tight text-slate-900">Resolution & Closure</CardTitle>
                             <CardDescription className="text-slate-400 font-medium">Document the fix and finalize the record.</CardDescription>
                          </div>
                       </div>
                    </CardHeader>
                    <CardContent className="px-8 pb-8 pt-4 space-y-10">
                       <div className="space-y-3">
                          <Label className="text-[10px] font-black text-slate-500 uppercase tracking-widest px-1">Resolution Strategy *</Label>
                          <textarea
                            name="resolution"
                            required
                            rows={4}
                       ...
                       <div className="grid grid-cols-1 md:grid-cols-2 gap-8 pt-8 border-t border-slate-50">
                          <div className="space-y-3">
                             <Label className="text-[10px] font-black text-slate-500 uppercase tracking-widest px-1">Root Cause Category</Label>
                             <Input 
                                name="rootCause"
                          ...
                          <div className="space-y-3">
                             <Label className="text-[10px] font-black text-slate-500 uppercase tracking-widest px-1">Total Labor Hours</Label>
                             <Input 
                                type="number"
                                step="0.5"
                       ...
                       <div className="space-y-3">
                          <Label className="text-[10px] font-black text-slate-500 uppercase tracking-widest px-1">Closure Notes (Private)</Label>
                          <textarea
                            name="closureNotes"
                            rows={2}
              ...
              <div className="space-y-10">
                 <Card className="rounded-[2.5rem] border-none shadow-xl bg-slate-900 text-white p-2">
                    ...
                    <CardContent className="px-8 py-8 space-y-8">
                       <div className="space-y-4">
                          <Label className="text-slate-400 font-black text-[10px] uppercase tracking-[0.2em]">Workflow Progress</Label>
                          <select 
                            name="status"
                       ...
                       <div className="space-y-4">
                          <Label className="text-slate-400 font-black text-[10px] uppercase tracking-[0.2em]">Response Priority</Label>
                 ...
                 <Card className="rounded-[2.5rem] border-none shadow-xl bg-white p-2 border-2 border-primary/5">
                    ...
                    <CardContent className="px-8 py-8 space-y-8">
                       <div className="space-y-3">
                          <Label className="text-[10px] font-black text-slate-500 uppercase tracking-widest px-1">Primary Engineer</Label>
                          <select
                            value={formData.assignedEngineerIds[0] || ""}
                            onChange={(e) => setFormData((prev: any) => ({ ...prev, assignedEngineerIds: e.target.value ? [e.target.value] : [] }))}
                       ...
                       <div className="space-y-3">
                          <Label className="text-[10px] font-black text-slate-500 uppercase tracking-widest px-1">Reschedule Date</Label>
                          <Input
                            type="date"
                            name="scheduledDate"
                 ...
                 <div className="p-8 bg-blue-50/50 rounded-[2.5rem] border-2 border-blue-100 flex items-start gap-4">
                    ...
                    <div className="space-y-2">
                       <p className="text-sm font-black text-blue-900 uppercase italic tracking-tight">Audit Preservation</p>
                       <p className="text-[11px] text-blue-700 font-medium leading-relaxed opacity-80">
                          State transitions are logged for SLA calculation and field force performance evaluation. Ensure data accuracy before locking the record.
                       </p>
                    </div>
                 </div>
              </div>
