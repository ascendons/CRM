...
export default function WorkOrderPartsPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const filteredProductResponses = searchQuery.length > 1 ? products.filter(p => 
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between py-5">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-xl font-black text-slate-900 tracking-tight uppercase">Consumption Log</h1>
                <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">{order.woNumber} • Spare Parts</p>
              </div>
            </div>
        ...
        <div className="space-y-10">
                   ...
                   {filteredProductResponses.map(p => (
                      ...
                      >
                         ...
                         <div className="flex-1">
                            <p className="text-sm font-black text-slate-900 tracking-tight capitalize">{p.productName}</p>
                            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{p.sku || 'No SKU'}</p>
                         </div>
           ...
           <div className="space-y-6">
              <div className="flex items-center justify-between px-2">
                 <div className="flex items-center gap-3">
                    ...
                    <Badge className="bg-slate-200 text-slate-600 border-none rounded-lg text-[10px] font-black px-2">{partsUsed.length} Lines</Badge>
                 </div>
              </div>
              ...
              {partsUsed.length === 0 ? (
                <div className="bg-white rounded-[3rem] p-24 text-center border-2 border-dashed border-slate-200">
                   ...
                   <h3 className="text-xl font-black text-slate-900 tracking-tight uppercase">No Parts Recorded</h3>
                   <p className="text-slate-500 font-medium max-w-xs mx-auto mt-2">Inventory consumption must be logged for accurate costing and stock levels.</p>
                </div>
              ) : (
                <div className="space-y-4">
                   {partsUsed.map((p, idx) => {
                      ...
                      return (
                        <Card key={idx} className="rounded-[2rem] border-none shadow-lg bg-white overflow-hidden group">
                           <CardContent className="p-6 sm:p-8 flex flex-col md:flex-row items-center gap-8">
                              ...
                              <div className="flex-1 min-w-0">
                                 <h4 className="text-lg font-black text-slate-900 tracking-tight truncate line-clamp-1">{product?.productName || p._name || 'Unknown Part'}</h4>
                                 <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">{product?.sku || p._sku || 'SKU-000'}</p>
                              </div>
                              ...
                              <div className="flex flex-wrap items-center gap-6">
                                 <div className="space-y-2">
                                    ...
                                    <div className="flex items-center bg-slate-50 rounded-xl border border-slate-100 overflow-hidden h-11">
                                       <button onClick={() => updatePart(p.partId, { qty: Math.max(1, (p.qty || 1) - 1) })} className="px-3 hover:bg-slate-200 transition-colors">
                                          <X className="h-3 w-3 text-slate-400 rotate-45" />
                                       </button>
                                       <input 
                                         type="number" 
                                         value={p.qty}
                                         onChange={(e) => updatePart(p.partId, { qty: parseInt(e.target.value) || 1 })}
                                         className="w-12 bg-transparent border-none text-center font-black text-slate-900 focus:ring-0 text-sm"
                                       />
                                       <button onClick={() => updatePart(p.partId, { qty: (p.qty || 1) + 1 })} className="px-3 hover:bg-slate-200 transition-colors">
                                 ...
                                 <div className="space-y-2 flex-1 md:flex-none">
                                    <Label className="text-[9px] font-black text-slate-400 uppercase px-1">Serial Number</Label>
                                    <Input 
                                      value={p.serialNo || ""}
           ...
           <div className="p-8 bg-blue-50/50 rounded-[3rem] border-2 border-blue-100/50 flex flex-col md:flex-row items-center gap-6">
              ...
              <div className="space-y-1 text-center md:text-left">
                 <p className="text-sm font-black text-blue-900 uppercase italic tracking-tight">Stock Valuation & Costing</p>
                 <p className="text-xs text-blue-700 font-medium leading-relaxed opacity-80">
                    Recording consumption here will automatically deduct items from the assigned Warehouse {order.assignedEngineerIds?.length ? "of the lead engineer" : "Default Location"} and impact final billing.
                 </p>
              </div>
           </div>
        </div>
