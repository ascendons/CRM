...
export default function WorkOrderDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const loadData = async () => {
    try {
      ...
      fieldService.getChecklistResponse(id).then(setChecklist).catch(() => setChecklist(null));
  ...
  const getPriorityBadge = (priority: WorkOrderPriority) => {
    switch (priority) {
      case WorkOrderPriority.LOW: return <Badge className="bg-slate-100 text-slate-600 border-slate-200 uppercase text-[10px] font-black tracking-widest px-3">Low</Badge>;
      case WorkOrderPriority.MEDIUM: return <Badge className="bg-blue-50 text-blue-700 border-blue-100 uppercase text-[10px] font-black tracking-widest px-3">Medium</Badge>;
      case WorkOrderPriority.HIGH: return <Badge className="bg-orange-50 text-orange-700 border-orange-100 uppercase text-[10px] font-black tracking-widest px-3">High</Badge>;
      case WorkOrderPriority.CRITICAL: return <Badge className="bg-rose-50 text-rose-700 border-rose-100 uppercase text-[10px] font-black tracking-widest px-3 animate-pulse">Critical</Badge>;
            ...
            <div className="flex items-center gap-6">
              ...
              <div>
                <div className="flex items-center gap-3 mb-1">
                   <h1 className="text-2xl font-black text-slate-900 tracking-tight uppercase">{order.woNumber}</h1>
                   {getPriorityBadge(order.priority)}
                </div>
                <div className="flex items-center gap-3 text-xs font-bold text-slate-500 uppercase tracking-widest">
                   <span className="flex items-center gap-1.5"><Clock className="h-3.5 w-3.5" /> {order.status.replace('_', ' ')}</span>
                   <span>•</span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-10">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                 <Card className="rounded-[2.5rem] border-none shadow-xl bg-white p-2 group hover:shadow-2xl transition-all">
                    <CardContent className="p-6 flex items-center gap-5">
                       ...
                       <div>
                          ...
                          <h4 className="text-lg font-black text-slate-900 tracking-tight leading-tight mb-1">{asset?.brand} {asset?.model}</h4>
                          <p className="text-xs font-bold text-slate-500">SN: {asset?.serialNo}</p>
                       </div>
                       <Button variant="ghost" size="icon" onClick={() => router.push(`/assets/${asset?.id}`)} className="ml-auto rounded-xl hover:bg-slate-100">
                 ...
                 <Card className="rounded-[2.5rem] border-none shadow-xl bg-white p-2 group hover:shadow-2xl transition-all">
                    <CardContent className="p-6 flex items-center gap-5">
                       ...
                       <div>
                          <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Service Level</p>
                          <h4 className="text-lg font-black text-slate-900 tracking-tight leading-tight mb-1">{contract?.type || "AD-HOC (BILLABLE)"}</h4>
                          <p className="text-xs font-bold text-slate-500">{contract ? `Ref: ${contract.contractNumber}` : "Standard on-demand pricing"}</p>
                       </div>
                       {contract && (
                         <Button variant="ghost" size="icon" onClick={() => router.push(`/contracts/${contract.id}`)} className="ml-auto rounded-xl hover:bg-slate-100">
                            <ExternalLink className="h-4 w-4" />
                         </Button>
                       )}
                    </CardContent>
              ...
              <Card className="rounded-[3rem] border-none shadow-xl bg-white p-4">
                 <CardHeader className="px-8 pt-8 pb-4">
                    <div className="flex items-center gap-3">
                       ...
                       <div>
                          <CardTitle className="text-2xl font-black text-slate-900 tracking-tight">Technical Analysis</CardTitle>
                          <CardDescription className="text-slate-400 font-medium">Original complaint and diagnostic findings.</CardDescription>
                       </div>
                    </div>
                 </CardHeader>
                 <CardContent className="px-8 pb-8 pt-6 space-y-10">
                    <div className="space-y-4">
                       <Label className="text-[10px] font-black font-bold uppercase tracking-[0.2em] text-slate-400">Reported Symptoms</Label>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                 ...
                 >
                    <CardContent className="p-8">
                       ...
                       <h5 className="text-lg font-black tracking-tight mb-2 uppercase italic">Safety Checklist</h5>
                       <p className="text-xs font-medium text-white/70 leading-relaxed">
                          {checklist?.completedAt ? "Checklist validated and stored." : "Mandatory inspection required before closure."}
                       </p>
                    </CardContent>
                 </Card>
                 ...
                 >
                    <CardContent className="p-8">
                       ...
                       <h5 className="text-lg font-black tracking-tight mb-2 uppercase italic">Spare Parts</h5>
                       <p className="text-xs font-medium text-white/70 leading-relaxed">
                 ...
                 <Card className="rounded-[2.5rem] bg-slate-100 text-slate-600 shadow-xl border-none">
                    <CardContent className="p-8">
                       ...
                       <h5 className="text-lg font-black tracking-tight mb-2 text-slate-900 uppercase italic">Timeline Logs</h5>
           ...
           <div className="space-y-10">
              <Card className="rounded-[3rem] border-none shadow-xl bg-white p-2">
                 <CardHeader className="px-8 pt-8 pb-4">
                    <CardTitle className="text-lg font-black uppercase tracking-tight">Dispatch Coordination</CardTitle>
                 </CardHeader>
                 <CardContent className="px-8 py-6 space-y-8">
                    <div className="space-y-4">
                       <Label className="text-[10px] font-black uppercase tracking-widest text-slate-400">Field Personnel</Label>
                       {order.assignedEngineerIds?.length ? (
                          order.assignedEngineerIds.map(eId => {
                             const eng = engineers.find(u => u.id === eId);
                             return (
                                <div key={eId} className="flex items-center gap-4 p-3 bg-slate-50 rounded-2xl border border-slate-100">
                                   ...
                                   <div>
                                      <p className="text-sm font-black text-slate-900 tracking-tight">{eng?.profile?.fullName || eng?.username}</p>
                                      <p className="text-[10px] font-bold text-slate-500 uppercase">{eng?.roleName}</p>
                                   </div>
                                </div>
                             )
                          })
                          ...
                          <div className="p-4 bg-amber-50 rounded-2xl border border-dashed border-amber-200 text-center">
                             ...
                             <p className="text-[10px] font-black text-amber-900 uppercase italic">Pending Allocation</p>
                          </div>
                       )}
                       <Button variant="outline" className="w-full rounded-xl h-12 font-bold text-xs uppercase tracking-widest text-slate-600 border-2 border-slate-50">
                          Re-Assign Team
                       </Button>
                    </div>
              ...
              <Card className="rounded-[3rem] border-none shadow-xl bg-slate-900 text-white p-2">
                 ...
                 <CardContent className="px-8 py-8 space-y-6">
                    <div className="flex items-center justify-between">
                       ...
                       <span className={`text-xs font-black italic ${order.slaBreached ? 'text-rose-500' : 'text-emerald-500'}`}>
              ...
              <div className="p-8 bg-blue-50/50 rounded-[3rem] border-2 border-blue-100/50 space-y-4">
                 ...
                 <p className="text-xs font-bold text-blue-900 uppercase tracking-tight">System Notice</p>
                 <p className="text-[11px] text-blue-700 font-medium leading-relaxed">
                    Closed orders are archived and frozen. Any subsequent issues require a new Re-Opened workflow or a fresh Service Request.
                 </p>
              </div>
           </div>
        </div>
