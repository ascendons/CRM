...
export default function WorkOrderChecklistPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const loadData = async () => {
    try {
      ...
      const templateData = await fieldService.getChecklistTemplateById(orderData.checklistTemplateId);
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between py-4">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-lg font-black text-slate-900 tracking-tight uppercase line-clamp-1">{template.name}</h1>
                <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">{order.woNumber} • Inspection</p>
              </div>
            </div>
          ...
          <Card className="rounded-[2.5rem] border-none shadow-xl bg-slate-900 text-white overflow-hidden">
             <CardContent className="p-8">
                <div className="flex items-start justify-between mb-6">
                   ...
                   <Badge variant="outline" className="border-white/20 text-white/60 font-black uppercase text-[9px] tracking-[0.2em] rounded-lg">
                      {template.jobType}
                   </Badge>
                </div>
                <h2 className="text-xl font-black tracking-tight mb-2 uppercase italic">Safety & Compliance Protocol</h2>
                <p className="text-sm font-medium text-white/60 leading-relaxed max-w-md">
                   Perform each step meticulously. Field data is audited for compliance and quality assurance.
                </p>
             </CardContent>
          </Card>
                       ...
                       <div className="flex-shrink-0 flex items-start gap-4">
                          ...
                          <div className="flex-1 space-y-1">
                             <div className="flex items-center gap-2">
                                ...
                                {item.isMandatory && <span className="text-rose-500 font-black text-lg leading-none mt-1">*</span>}
                             </div>
                             <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{item.itemCode}</p>
                          </div>
                       </div>
                          ...
                          {item.inputType === 'PASS_FAIL' ? (
                            <div className="flex bg-slate-100 p-1.5 rounded-[1.25rem] border-2 border-slate-50 gap-1">
                               <button 
                                 ...
                                 onClick={() => handleItemUpdate(item.itemCode, { status: ChecklistItemStatus.PASS })}
                               ...
                               <button 
                                 type="button"
                                 disabled={isCompleted}
                                 onClick={() => handleItemUpdate(item.itemCode, { status: ChecklistItemStatus.FAIL })}
                                 className={`flex-1 h-11 px-6 rounded-xl flex items-center justify-center gap-2 text-[10px] font-black uppercase tracking-widest transition-all ${
                            ...
                            <Input 
                              ...
                              disabled={isCompleted}
                              value={res?.value || ""}
                              onChange={(e) => handleItemUpdate(item.itemCode, { value: e.target.value })}
                    ...
                    <div className="mt-8 pt-6 border-t border-slate-50 flex flex-wrap gap-4">
                       ...
                       <div className="flex-1 relative">
                          ...
                          <input 
                            ...
                            disabled={isCompleted}
                            value={res?.note || ""}
                            onChange={(e) => handleItemUpdate(item.itemCode, { note: e.target.value })}
          ...
          {!isCompleted && (
            <div className="pt-10">
               ...
               <p className="text-center mt-6 text-[10px] font-bold text-slate-400 uppercase tracking-widest px-10 leading-relaxed italic">
                  By submitting, you certify that all safety checks were performed according to protocol and standard operating procedures.
               </p>
            </div>
          )}
        </div>
