      ...
      <main className="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {loading ? (
          ...
          <div className="bg-white rounded-[3rem] p-32 text-center shadow-2xl border-2 border-dashed border-slate-100 flex flex-col items-center animate-in zoom-in-95 duration-500">
            ...
            <p className="text-slate-400 font-bold max-w-md mx-auto mb-10 text-lg leading-relaxed">
              No work orders match your current filters. Adjust your search or dispatch a new order
              to the field.
            </p>
            <Button
              onClick={() => router.push("/work-orders/new")}
              className="rounded-[2rem] h-16 px-12 font-black shadow-2xl shadow-primary/25 text-lg"
            >
                ...
                <div key={status} className="flex-shrink-0 w-80 lg:w-96 flex flex-col gap-6">
                  <div className="flex items-center justify-between px-2">
                    <div className="flex items-center gap-3">
                      <div
                        className={`h-2.5 w-2.5 rounded-full ${status === WorkOrderStatus.OPEN ? "bg-slate-300" : "bg-primary shadow-[0_0_10px_rgba(var(--primary-rgb),0.5)]"}`}
                      ></div>
                      <h3 className="text-[11px] font-black uppercase tracking-[0.25em] text-slate-950">
                        {getStatusLabel(status)}
                      </h3>
                      <Badge className="bg-slate-200 text-slate-600 rounded-lg text-[10px] font-black px-2">
                        {columnOrders.length}
                      </Badge>
                    </div>
                  </div>
                        ...
                        >
                          <CardContent className="p-6">
                            <div className="flex items-start justify-between mb-5">
                              <Badge
                                className={`${getPriorityColor(order.priority)} rounded-xl text-[9px] font-black uppercase tracking-widest px-3 py-1 border-2`}
                              >
                                {order.priority}
                              </Badge>
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-slate-900 border-b border-white/5">
                    <th className="px-8 py-6 text-[10px] font-black text-slate-400 uppercase tracking-[0.25em]">
                      Track ID
                    </th>
                    <th className="px-8 py-6 text-[10px] font-black text-slate-400 uppercase tracking-[0.25em]">
                      Status & Priority
                ...
                <tbody className="divide-y divide-slate-50">
                  {filteredOrders
                    .sort((a, b) => b.woNumber.localeCompare(a.woNumber))
                    .map((order) => {
                      const eng = engineers.find((e) => order.assignedEngineerIds?.includes(e.id));
                      return (
                        <tr
                        ...
                        >
                          <td className="px-8 py-7">
                            <span className="text-sm font-black text-slate-950 tracking-tighter uppercase">
                              {order.woNumber}
                            </span>
                          ...
                          <td className="px-8 py-7">
                            <div className="flex flex-col gap-2">
                              <div className="flex items-center gap-2">
                                <div
                                  className={`h-1.5 w-1.5 rounded-full ${order.status === WorkOrderStatus.OPEN ? "bg-slate-300" : "bg-primary"}`}
                                ></div>
                              ...
                              <Badge
                                className={`${getPriorityColor(order.priority)} rounded-lg text-[9px] font-black px-2 py-0.5 border w-fit`}
                              >
                                {order.priority}
                              </Badge>
                            </div>
                          ...
                          <td className="px-8 py-7">
                            ...
                            <div className="flex items-center gap-1.5 mt-0.5">
                              {order.slaBreached ? (
                                <Badge
                                  variant="outline"
                                  className="text-[9px] font-black text-rose-600 border-rose-200 bg-rose-50 rounded-md"
                                >
                                  BREACHED
                                </Badge>
                              ) : (
                          ...
                          <td className="px-8 py-7 text-right">
                            <Button
                              variant="ghost"
                              size="icon"
                              className="h-10 w-10 rounded-xl group-hover:bg-primary group-hover:text-white transition-all shadow-none"
                            >
