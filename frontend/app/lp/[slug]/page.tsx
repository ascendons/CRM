...
export default function PublicLandingPage() {
  ...
  useEffect(() => {
    if (slug) {
      formsService
        .getLandingPageBySlug(slug as string)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setSubmitting(true);
      await fetch(
        `${process.env.NEXT_PUBLIC_API_URL || "http://localhost:8080/api/v1"}/forms/${page.formId}/submit`,
        {
          method: "POST",
      ...
      <div className="bg-blue-600 text-white py-20 px-6 text-center">
        {page.heroImageUrl && (
          <img
            ...
            alt={page.title}
            className="w-full max-w-2xl mx-auto rounded-xl mb-8 object-cover max-h-64"
          />
        )}
        <h1 className="text-4xl font-bold mb-4">{page.title}</h1>
        {page.heroText && (
          <p className="text-xl text-blue-100 max-w-2xl mx-auto">{page.heroText}</p>
        )}
        {!form && page.ctaText && (
          <a
            href="#form"
            className="inline-block mt-8 bg-white text-blue-600 font-semibold px-8 py-3 rounded-full hover:bg-blue-50 transition"
          >
            {page.ctaText}
          </a>
        )}
                  ...
                  {field.type === "TEXTAREA" ? (
                    <textarea
                      ...
                      onChange={(e) =>
                        setResponses((r) => ({
                          ...r,
                          [field.fieldId || field.label]: e.target.value,
                        }))
                      }
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setResponses((r) => ({
                          ...r,
                          [field.fieldId || field.label]: e.target.checked ? "true" : "false",
                        }))
                      }
                      className="rounded"
                    />
