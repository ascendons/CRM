...
export default function ChatPanel({ isOpen, onClose }: ChatPanelProps) {
  ...
  const handleSendMessage = (e: React.FormEvent) => {
    ...
    if (selectedRecipientId && selectedRecipientType) {
      sendTypingIndicator(
        selectedRecipientId,
        selectedRecipientType === "GROUP" ? "GROUP" : "USER",
        false
      );
    }
                  ...
                  <div className="flex h-full flex-col bg-slate-50 shadow-xl overflow-hidden">
                    <div className="px-4 py-4 border-b border-slate-200 bg-white sm:px-6 flex items-center justify-between z-10">
                      ...
                      <div className="flex items-center gap-4">
                        <span
                          className={`flex items-center gap-1.5 text-xs font-medium px-2.5 py-1 rounded-full ${connected ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"}`}
                        >
                          <span
                      ...
                      <div className="w-[70px] sm:w-1/3 border-r border-slate-200 bg-white overflow-y-auto shrink-0 transition-all duration-200">
                        ...
                        <div className="px-3 py-2 border-t border-slate-200">
                          <div className="flex items-center justify-center sm:justify-between mb-2">
                            <h3 className="text-[10px] sm:text-xs font-semibold text-slate-400 tracking-wider uppercase hidden sm:block">
                              Custom Groups
                            </h3>
                            <button
                                ...
                                >
                                  ...
                                  {unreadMessageCounts[group.id] > 0 && (
                                    <span className="shrink-0 bg-red-500 text-white text-[10px] font-bold px-2 py-0.5 rounded-full">
                                      {unreadMessageCounts[group.id] > 9
                                        ? "9+"
                                        : unreadMessageCounts[group.id]}
                                    </span>
                                  )}
                                </button>
                              </li>
                        ...
                        <div className="px-3 py-2 border-t border-slate-200">
                          <h3 className="text-[10px] sm:text-xs font-semibold text-slate-400 tracking-wider uppercase mb-2 hidden sm:block">
                            Direct Messages
                          </h3>
                          <ul className="space-y-1">
                            {users.map((user) => (
                                ...
                                activeConversationMessages.map((msg, idx) => {
                                  ...
                                  return (
                                    ...
                                    >
                                      ...
                                      <div
                                        className={`max-w-[85%] px-3 py-2 rounded-2xl text-sm ${isMe ? "bg-primary text-white rounded-tr-none" : "bg-white border border-slate-200 text-slate-800 rounded-tl-none"}`}
                                      >
                                        {msg.content}
                                      </div>
                      ...
                      <div className="px-4 py-5 sm:p-6 space-y-4">
                        ...
                        <div>
                          ...
                          <p className="mt-1 text-xs text-slate-500">
                            You will automatically be added to the group.
                          </p>
                        </div>
                      </div>
